...
void command_task(void *pvParameter) {
    ...
    while(1) {
        if (xQueueReceive(cmd_queue, &cmd, portMAX_DELAY)) {
            ...
            // Saved Presets
            else if (commandStr == "ZERO_G") { maxWait = bed.setTarget(bed.getSavedPos("zg_head", 10000), bed.getSavedPos("zg_foot", 40000)); activeCommandLog = "ZERO_G"; }
            else if (commandStr == "ANTI_SNORE") { maxWait = bed.setTarget(bed.getSavedPos("snore_head", 10000), bed.getSavedPos("snore_foot", 0)); activeCommandLog = "ANTI_SNORE"; }
            else if (commandStr == "LEGS_UP") { maxWait = bed.setTarget(bed.getSavedPos("legs_head", 0), bed.getSavedPos("legs_foot", 43000)); activeCommandLog = "LEGS_UP"; }
            else if (commandStr == "P1") { maxWait = bed.setTarget(bed.getSavedPos("p1_head", 0), bed.getSavedPos("p1_foot", 0)); activeCommandLog = "P1"; }
            else if (commandStr == "P2") { maxWait = bed.setTarget(bed.getSavedPos("p2_head", 0), bed.getSavedPos("p2_foot", 0)); activeCommandLog = "P2"; }

            // --- SAVE LOGIC ---
            else if (commandStr.rfind("SET_", 0) == 0) {
...
extern "C" void app_main() {
    ...
    xTaskCreatePinnedToCore(command_task, "command_task", 4096, NULL, 5, NULL, 0);
}
