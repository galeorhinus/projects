...
- Five-section structure preserved with sharper titles (e.g., §4.1 *The Grammar Before the Grammar* tightens current's *The Mahābhāṣya — The Great Commentary*).
- §4.1 prose compressed substantially (current ~1,100 words → Codex ~480 words). All load-bearing content kept: pre-Pāṇinian roster with Devanagari, Trimuni Vyākaraṇam, standing polemic phrase, *vyākaraṇam* etymology, *vaiyākaraṇa* role-title, sthapati/nirmātṛ contrast.
- *pañca prayojanāni* relocated from §4.1 to §4.2 (where the Paspaśāhnika is introduced) — cleaner flow.
- §4.4 *codification-vocabulary-fails* paragraph adopted (Codex addition): tighter than the current chapter's equivalent. *"Codification imagines drift first and order later. Patañjali gives the opposite: established bond first, grammatical defense afterward."*
- §4.3 compression preserves the *siddha* / *kārya* contrast, the cross-disciplinary anchoring, the two-branch development, and the *apabhraṃśa* introduction.
- §4.5 chapter close (*"Without siddha, there is nothing to defend"*) preserved verbatim.
- The "list isn't exhaustive" caveat dropped — Codex's "*are the documentary trace of a discipline already operating*" does the work.
- The *pūrvapakṣa* / *uttarapakṣa* technical-vocabulary detail at §4.4 ¶3 dropped — Codex's "*the ordinary Indic disputational structure: the opposing position is stated, the defending position answers, and the resolution is reached*" carries the same content in plain English.

**Endnote stubs in this chapter:** `shakalya-padapatha`, `panini-cites-pre-paninian-grammarians`, `vyakarana-etymology`, `vaiyakarana-role-title`, `prayojanani-paspashahnika`, `panini-no-preface`, `siddhe-shabdarthasambandhe-mbh`.
