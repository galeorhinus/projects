...
Second, the dates the modern academy has assigned to Indic figures and events come from the same comparative-philological apparatus this book is questioning. Pāṇini's “500 BCE” or “400 BCE” or any other dating is not neutral evidence. It is a product of the apparatus that needed Sanskrit to sit inside a chronological sequence it could authorize, teach, cite, and defend. The apparatus arranged the evidence inside its own assumptions, then called the arrangement *chronology.* This book does not carry forward dates produced by a machinery that misread Sanskrit itself.

The asymmetry is not global mistrust. The non-Indic dates above are internal to traditions that operate chronologically without distorting their own categories; they stand. The mistrust is local to philological dating of Indic events. I have not extended it to areas that are neither my interest nor my domain of expertise.`\textsuperscript{[4]}`{=latex}
...
The tongue is the most complex moving part: tip, blade, body, and base. Above it sit the upper teeth, the alveolar ridge, the hard palate, the soft palate, and the uvula. At the front are the lips. At the back is the throat. The distance from lips to glottis averages roughly seventeen centimeters in adult males, ~14–15 cm in adult females, and shorter still in children, with the adult range running from about 13 cm to about 20 cm. The architecture is the same.`\textsuperscript{[57]}`{=latex}
...
That unit is the **`{\devanagarifont अक्षर}`{=latex} (*akṣara*)** — the imperishable. The morphology is exact: *a-* (privative) + √*kṣar* (to flow, to perish) — *that which does not decay*. The name is not neutral. Latin has *littera*, a mark. Greek has *gramma*, what is written. Arabic has *ḥarf*, an edge. Sanskrit names the writing-and-utterance primitive *akṣara*, the imperishable. The non-decay claim sits inside the name.`\textsuperscript{[73]}`{=latex}
...
The vocabulary developed here is not modern explanation imposed backward. *Sthāna*, *karaṇa*, *prayatna*, *prāṇa*, *ghoṣa*, *anunāsika*, *sparśa*, *swara*, *antaḥstha*, *ūṣman*, *varga*, *varṇa*, and *varṇamālā* belong to the Sanskrit grammatical and phonetic disciplines themselves. The *Prātiśākhya* and *Śikṣā* materials document the architecture as already operating vocabulary.`\textsuperscript{[74]}`{=latex}`\textsuperscript{[75]}`{=latex}
...
Frits Staal saw the structural point clearly when he compared the *varga* system to Mendeleev's periodic table.`\textsuperscript{[76]}`{=latex} The comparison is right. Each *sparśa* consonant sits at a coordinate determined by constituent properties, just as a chemical element sits inside a reference system determined by its constituent properties.
...
The same logic extends beyond *sparśa*. *Swaras* sample vowel-space and add time: *hrasva*, *dīrgha*, and *pluta*.`\textsuperscript{[82]}`{=latex} *Antaḥstha* occupy the in-between mode. *Ūṣman* operate constricted breath. *Ayogavāha* specifies breath termination. The full *varṇamālā* is not a list. It is the integrated specification of the speaking body.
...
The southern languages — Tamil, Kannada, Malayalam, Telugu, Tulu — preserve the five-zone place axis and the retroflex series. They do not operate the full *mahāprāṇa* series natively in the same way Sanskrit does; aspirates generally enter through Sanskritic vocabulary.`\textsuperscript{[85]}`{=latex} That difference matters. These languages are not failed Sanskrit. They are parallel selections from the same regional substrate.
...
The eastern subcontinent — Bengali, Odia, Assamese, Maithili's eastern edge — preserves the retroflex series and the five-zone axis. Bengali's `{\devanagarifont व}`{=latex} / `{\devanagarifont ब}`{=latex} merger occurs inside the labial zone, exactly where the matrix carries a close neighbor. Odia remains especially conservative in its consonant inventory.`\textsuperscript{[86]}`{=latex}

The Sindhu region adds the cleanest exclusion case. Sindhi preserves retroflexion, the five-zone axis, and aspiration, but also operates implosives — ɓ, ɗ, ʄ, ɠ — that Sanskrit does not include.`\textsuperscript{[87]}`{=latex} The sound-field contains them. The *varṇamālā* rejects them.
...
Now compare the matrix with the broader superset. The subcontinental field contains more than the five Sanskrit places. Tamil operates an alveolar contact-station distinct from dental and retroflex.`\textsuperscript{[88]}`{=latex} Ho, Mundari, and related languages operate glottal stops or checked consonants.`\textsuperscript{[89]}`{=latex} Persian-and-Arabic-influenced registers carry labio-dental fricatives such as *f*.`\textsuperscript{[90]}`{=latex} Sindhi operates implosives. Some regional systems carry fine distinctions around the palatal zone.
...
The **`{\devanagarifont शिक्षा}`{=latex} (*Śikṣā*)** discipline gives Sanskrit a timing grid. The unit is the **`{\devanagarifont मात्रा}`{=latex} (*mātrā*)** — a measure of duration. The system does not merely ask where the tongue strikes or how the breath moves. It asks how long the sound holds.`\textsuperscript{[91]}`{=latex}
...
Punjabi confirms the danger. Its tones developed after voiced aspirates weakened or merged; the lost consonantal contrast leaked into neighboring vowels as pitch.`\textsuperscript{[92]}`{=latex} Some Pahari languages show related tonal developments.`\textsuperscript{[93]}`{=latex} Tone here is evidence of drift after an older consonantal contrast weakened. Sanskrit's preserved grid prevents that drift.
...
**Sindhi implosives.** Sindhi carries implosive consonants. Sanskrit does not. Implosives are voiced stops with inward airflow, acoustically close enough to ordinary voiced stops to crowd the row. The field contains them; the grid rejects them.`\textsuperscript{[94]}`{=latex}

**Tamil alveolar.** Tamil preserves an alveolar contact-station between dental and retroflex. Sanskrit excludes it. The tongue-tip axis would become too crowded if dental, alveolar, and retroflex were all full grid rows.`\textsuperscript{[95]}`{=latex}

**Glottal stop.** Some central-eastern languages use glottal closure phonemically. Sanskrit does not. It assigns the glottal/breath region to different work: *visarga* and related breath-gestures.`\textsuperscript{[96]}`{=latex}

**Labio-dental fricatives.** The *f* / *v* region appears through contact registers and in non-subcontinental languages. Sanskrit keeps the labial row bilabial. It does not crowd the front edge of the mouth with a second labial contact-station.`\textsuperscript{[97]}`{=latex}
...
The evidence is visible in the architecture. No signed plan is needed for the same reason no signed plan is needed to recognize the engineering of Kailasa at Ellora. The temple's architects are anonymous; the temple's engineering is not.`\textsuperscript{[99]}`{=latex} The rock carries the record.
...
![Ten *dhātavaḥ* across the *mātrā* envelope, in half-*mātrā* steps from 1 to 5½. `{\devanagarifont ऋ}`{=latex} (*ṛ*) sits at the floor — a bare short vowel, the minimum atom; `{\devanagarifont ऋषि}`{=latex} (*ṛṣi*) and `{\devanagarifont ऋत}`{=latex} (*ṛta*) descend from it. `{\devanagarifont कृ}`{=latex} (*kṛ*) adds one consonant and builds `{\devanagarifont कर्म}`{=latex} and `{\devanagarifont कार्य}`{=latex}. `{\devanagarifont गम्}`{=latex} (*gam*) at 2 *mātrās* is the modal envelope — `{\devanagarifont गमन}`{=latex}, `{\devanagarifont आगम}`{=latex}. `{\devanagarifont धा}`{=latex} (*dhā*) builds `{\devanagarifont धातु}`{=latex} and `{\devanagarifont विधा}`{=latex}; `{\devanagarifont वाच्}`{=latex} (*vāc*) builds `{\devanagarifont वाचा}`{=latex} and `{\devanagarifont वाणी}`{=latex}. `{\devanagarifont स्वाद्}`{=latex} (*svād*) builds `{\devanagarifont स्वादिष्ट}`{=latex}; `{\devanagarifont बाधृ}`{=latex} (*bādhṛ*) builds `{\devanagarifont बाधा}`{=latex}; `{\devanagarifont कुमार्}`{=latex} (*kumār*) builds `{\devanagarifont कुमार}`{=latex} and `{\devanagarifont कौमार}`{=latex}. `{\devanagarifont दीपी}`{=latex} (*dīpī*) at 5 *mātrās* is the lamp-root — `{\devanagarifont दीप}`{=latex}, `{\devanagarifont दीपावली}`{=latex}, `{\devanagarifont प्रदीप}`{=latex}. `{\devanagarifont ह्लादी}`{=latex} (*hlādī*) at 5½ sits at the cliff and builds `{\devanagarifont आह्लाद}`{=latex}.](figures/build/building_dhatuh_matra_envelope.svg){#fig:building-dhatuh-matra-envelope width=95%}
...
The empirical distribution confirms it. The 2-*mātrā* envelope alone carries **886 entries — 40.87%** of the inventory. The 2½-*mātrā* atoms add another **520 (23.99%)**, lifting cumulative coverage past **71%**; through 3 *mātrās* the cumulative reaches **82%**. A secondary peak at 4 *mātrās* (**188 entries, 8.67%**) marks the disyllabic family — atoms like `{\devanagarifont बाधृ}`{=latex} (*bādhṛ*). Past 5 *mātrās* the cliff begins: every value from 5 onward sums to under **2%** of the corpus. Three *mātrā* values carry the inventory; the architecture commits to a narrow band of measured time.

## 10.5 *Dhāturacanā* — The Atomic Scaffold

The next step is abstraction.

Specific *dhātavaḥ* fill specific sounds into a repeatable measured scaffold. That scaffold is a **`{\devanagarifont धातुरचना}`{=latex} (*dhāturacanā*)** — a constructed arrangement for a *dhātuḥ*. `{\devanagarifont गम्}`{=latex} (*gam*), `{\devanagarifont नम्}`{=latex} (*nam*), `{\devanagarifont पच्}`{=latex} (*pac*), `{\devanagarifont वद्}`{=latex} (*vad*) are different atoms, but they inhabit the same scaffold: **CV1C**. One consonantal event, one short-vowel nucleus, one consonantal event. The timing is already inside the scaffold.

![Four distinct *dhātavaḥ* — `{\devanagarifont गम्}`{=latex} (*gam*), `{\devanagarifont नम्}`{=latex} (*nam*), `{\devanagarifont पच्}`{=latex} (*pac*), `{\devanagarifont वद्}`{=latex} (*vad*) — all inhabit the same **CV1C** atomic scaffold. The filled hexagons differ in *varṇa*; the scaffold on the right is identical. *Varṇāḥ* fill the slots; the slots themselves are the engineered timing-grid the architecture commits to.](figures/build/building_dhatuh_racana_scaffold.svg){#fig:building-dhatuh-racana-scaffold width=85%}
...
The empirical distribution is decisive. The **`{\devanagarifont धातुपाठ}`{=latex} (*Dhātupāṭha*)** lists 2,168 *dhātavaḥ* across ten **`{\devanagarifont गणाः}`{=latex} (*gaṇāḥ*)**. After **`{\devanagarifont अनुबन्ध}`{=latex} (*anubandha*)** stripping, the corpus inhabits 69 observed *racanā* scaffolds. But the distribution is not flat. Ten scaffolds carry **81.27%** of the corpus.`\textsuperscript{[100]}`{=latex}

![The top 10 *racanāḥ* by count across the 2,168-entry *Dhātupāṭha*. The y-axis lists the scaffold's structural shorthand (CV1C, CCV1C, …); each bar's length is the entry count, with count and corpus share to the right of the bar and the *racanā* name (Devanagari + Roman) and its *mātrā* value beyond. The `{\devanagarifont गमादि}`{=latex} (*gamādi*) scaffold alone carries **37.78%** of the corpus; ten scaffolds together cover **81.27%**.](figures/build/building_dhatuh_top_ten_racanas.svg){#fig:building-dhatuh-top-ten-racanas width=95%}
...
Compression also predicts generativity. The most productive *dhātavaḥ* are structurally small. *Kṛ* anchors 75+ primary derivatives; *bhū*, *dā*, *dhā*, *jñā*, *hṛ*, *nī* each generate 40–55 derivatives apiece.`\textsuperscript{[101]}`{=latex}
...
Yaska's decoding of **`{\devanagarifont अग्नि}`{=latex} (*agni*)** at *Nirukta* 7.14 makes the point concrete. The text predates Pāṇini and is foundational to the etymological decoding discipline. Yaska treats one word with four independent *dhātu*-level decodings, two attributed to earlier named pre-Pāṇinian decoders:`\textsuperscript{[104]}`{=latex}
...
He named the ten **`{\devanagarifont गणाः}`{=latex} (*gaṇāḥ*)**, the inflectional classes. He documented the **`{\devanagarifont विकरण}`{=latex} (*vikaraṇa*)**, the column signature that distinguishes one conjugational pattern from another.`\textsuperscript{[105]}`{=latex} He showed how *dhātavaḥ* enter the grammar's molecular machinery.
...
Mendeleev organized the chemical elements by structural property; chemical behavior emerged from position in the table.`\textsuperscript{[106]}`{=latex} He did not sort elements by what they "meant." He sorted them by behavior, and the chemistry became visible.
...
*Kṛ* (`{\devanagarifont कृ}`{=latex}, to make) carries valency 1,062 in the Digital Corpus of Sanskrit.`\textsuperscript{[107]}`{=latex} *Bhū* (`{\devanagarifont भू}`{=latex}, to be) carries 504. *Dhā* (`{\devanagarifont धा}`{=latex}, to place) carries 386. These are not claims about prestige or usefulness. They are corpus-attested bonding counts.
...
The periodic-table column for an atom is the column of its first consonant.`\textsuperscript{[109]}`{=latex} Semivowels, sibilants, and `{\devanagarifont ह}`{=latex} function as additional structural columns. Vowel-initial atoms occupy a separate band.
...
The named *Śikṣā* texts are compact because they are training manuals, not survey essays. The *Pāṇinīya-Śikṣā*, *Yājñavalkya-Śikṣā*, *Vāsiṣṭhī-Śikṣā*, *Āpiśali-Śikṣā*, and *Bhāradvāja-Śikṣā* belong to that world of disciplined production.`\textsuperscript{[119]}`{=latex} They read like operating instructions for a precision instrument. Points of articulation are enumerated. Vowel durations are quantified in **`{\devanagarifont मात्रा}`{=latex} (*mātrā*)** counts; consonants are fixed at half a *mātrā* — ***`{\devanagarifont अर्धमात्रा}`{=latex} `{\devanagarifont तु}`{=latex} `{\devanagarifont व्यञ्जनम्}`{=latex}***.`\textsuperscript{[120]}`{=latex} Modes of contact between articulators are specified. Consequences of slippage at each point are tabulated. The mechanical character of the texts is the mark of the architecture working through them.
...
This is why the Veda is not a written text first and a recitation second. The recitation is the primary body. Writing is a later reflection. The standard enumeration of the *Vedāṅgas* preserves the priority correctly: *Śikṣā* stands first.`\textsuperscript{[121]}`{=latex} Sound-production precedes the grammar that analyzes the form, the etymology that explains it, the ritual that employs it, and the calendrical discipline that times its use.

Auditure is the foundation. *Śikṣā* trains the body that carries it.

## 15.2 The Eleven *Pāṭhas*

The eleven *pāṭhas* divide into two groups. The five *prakṛti-pāṭhas* are primary recitations. The six *vikṛti-pāṭhas* are modified recitations that apply deeper permutations. Together they form one of the densest preservation codes any civilization has produced.`\textsuperscript{[122]}`{=latex}
...
Vedic recitation survives in multiple geographically separated lineages: the Nambūdiri Brahmins of Kerala; Maharashtra Brahmin lineages; Tamil Nadu and Karnataka lineages; the Banaras and Allahabad lineages of the northern plains; Gujarat and Rajasthan lineages of the western coast; the Kashmir Pandit lineages preserved through displacement.`\textsuperscript{[126]}`{=latex} These communities did not operate as one centralized institution and have no documented contact across most of the cross-lineage matrix. They preserved through parallel *guru-shishya* chains.

The recitations can be compared. Recordings exist. Frits Staal's fieldwork on Nambūdiri recitation in the 1970s placed recitation traditions into audio and film archives.`\textsuperscript{[127]}`{=latex} Later recordings extend the corpus. Where lineages differ, the differences are named, located, and governed by *śākhā* specification. They are not random drift.

The phonetic constants match at the level the architecture predicts: *varṇa* inventory, accent where preserved, *sandhi* execution, metrical structure, and recitational discipline. The lineages become independent witnesses. Their testimony agrees.`\textsuperscript{[128]}`{=latex}

UNESCO recognized Vedic chanting in 2003 as a Masterpiece of the Oral and Intangible Heritage of Humanity.`\textsuperscript{[129]}`{=latex} The recognition is not the authority. The recitation is the authority. But the citation matters because even an external institution had to acknowledge the continuity and precision of the practice in front of it.
...
The priests of progress have been writing this story for seventy years.`\textsuperscript{[132]}`{=latex} One scholar made the retroflex the seminal areal-feature exhibit. Another carried the position into the standard textbook account. A third pushed a Munda-substrate variant when the Dravidian version came under empirical strain. A fourth, from inside the framework, argued that the substrate hypothesis was not supported by the data. The dissent was filed and ignored. The textbook line continued.
...
The chapter's epigraph states the site directly. The meaning is simple: `{\devanagarifont मूर्धा}`{=latex} (*mūrdhā*) — the head / roof of the mouth — is the articulatory site for `{\devanagarifont ऋ}`{=latex} (*ṛ*), the `{\devanagarifont टु}`{=latex} (*ṭu*) class, `{\devanagarifont र}`{=latex} (*ra*), and `{\devanagarifont ष}`{=latex} (*ṣa*).`\textsuperscript{[134]}`{=latex} But the line does more than state the rule. Its own sound-body performs it. In the operative word `{\devanagarifont ऋटुरषाणां}`{=latex} (*ṛṭuraṣāṇāṃ*), the first vowel is `{\devanagarifont ऋ}`{=latex} (*ṛ*); the consonantal spine is `{\devanagarifont ट}`{=latex}-`{\devanagarifont र}`{=latex}-`{\devanagarifont ष}`{=latex}-`{\devanagarifont ण}`{=latex} (*ṭ-r-ṣ-ṇ*) — retroflex, semi-retroflex, retroflex, retroflex. The mouth must enter the site to say the rule that names the site. Sanskrit's phonetic science does not merely describe the architecture from outside. It makes the speaker perform the architecture while naming it.
...
Sanātan's own primary-source authority shows the opposite of what AIT claims. The **`{\devanagarifont असलायनसुत्त}`{=latex} (*Assalāyana Sutta*)** of the **`{\devanagarifont मज्झिमनिकाय}`{=latex} (*Majjhima Nikāya*)** preserves a dialogue in which the Buddha, responding to Assalāyana on the question of *varṇa* (`{\devanagarifont वर्ण}`{=latex}), observes that the bordering nations of **`{\devanagarifont योन}`{=latex} (*Yona*)** and **`{\devanagarifont कम्बोज}`{=latex} (*Kamboja*)** — the Greek and Iranian-frontier regions immediately outside the dharmic subcontinent — operate with only two varnas, *ārya* and *dāsa*, where the dharmic-Indic frame has the full fourfold *varṇa* system.`\textsuperscript{[140]}`{=latex}
...
That was the force of the moment. Savarkar did not need to utter the word; the audience completed it for him. The monitoring machinery of the British colonial state could not charge the speaker for a word the speaker had not spoken.`\textsuperscript{[141]}`{=latex}`\textsuperscript{[142]}`{=latex}
...
The origin question still admits two speculations. Both begin from the same epistemic position: nobody knows what is upstream of Sanskrit. One speculation admits this. The other hides it.`\textsuperscript{[144]}`{=latex}
...
> akin to Old High German *muoter*, Latin *māter*, Greek *m`{\symbolfont ē}`{=latex}t`{\symbolfont ē}`{=latex}r*, Sanskrit *mātṛ*`\textsuperscript{[146]}`{=latex}
...
And in early-19th-century philology — across the foundational works of the comparative-method machinery as it was being built`\textsuperscript{[151]}`{=latex} — Sanskrit was routinely placed at or near the source position of Indo-European etymological chains.
...
The orthodox account has a stock deflection for the *mother* case: the "nursery word" argument — the universal infant-babble cluster of *mama*-type forms across unrelated languages (Roman Jakobson, "Why 'Mama' and 'Papa'?", 1959), treated as a phonetic universal rather than evidence of cognation.`\textsuperscript{[153]}`{=latex}
...
The standard account (Mayrhofer, *Etymologisches Wörterbuch des Altindoarischen*) pushes Sanskrit *asura-* through Indo-Iranian **\*asura-** toward a *contested* PIE **\*h`{\symbolfont ₂}`{=latex}nsu-** meaning "life force" or "lord."`\textsuperscript{[156]}`{=latex}
...
> *"The root is suppletive — although `{\devanagarifont पश्यति}`{=latex} (paśyati) derives from Proto-Indo-European \*spe`{\symbolfont ḱ}`{=latex}-, the remainder of the morphological paradigm is derived from Proto-Indo-European \*der`{\symbolfont ḱ}`{=latex}-."*`\textsuperscript{[157]}`{=latex}
...
Old Persian supplies the contrast. The Behistun inscription, transliterated into Devanagari, can be read today by a fluent student of Sanskrit.`\textsuperscript{[160]}`{=latex} The grammar, the dhātu structures, much of the vocabulary — all of it sits within recognizable distance of the language a modern Indian student is trained in. A fluent modern speaker of Persian, presented with the same text, does not have the same experience. The two languages were close kin in their attested forms. They have ended up in radically different relationships to their own past.
...
The calibrant framework has three phases.`\textsuperscript{[166]}`{=latex}
...
The dates are the spine. **Franz Bopp** (1791–1867), trained at Paris under Antoine-Léonard de Chézy and at London under Henry Thomas Colebrooke, published *Über das Conjugationssystem der Sanskritsprache in Vergleichung mit jenem der griechischen, lateinischen, persischen und germanischen Sprache* in 1816.`\textsuperscript{[174]}`{=latex} The work treats Sanskrit verbal morphology as the structural anchor against which Greek, Latin, Persian, and Germanic verbal systems are compared. Bopp's *Vergleichende Grammatik der Sanskrit-, Send-, Armenischen-, Griechischen-, Lateinischen-, Litauischen-, Altslavischen-, Gothischen- und Deutschen* (1833–1852) carried the comparison across the family. Sanskrit was still the anchor; the comparative method was being built around it.
...
The apparatus assigns relative dates to Sanskrit texts partly through linguistic features. A text with more "archaic" features is earlier. A text with fewer "archaic" features is later. The sequence is then used to prove the language changed from archaic to later. The conclusion is folded into the method that produced the sequence.`\textsuperscript{[180]}`{=latex}
...
The bond comes first. Usage comes after. *Śāstra* regulates usage. It does not manufacture the bond.`\textsuperscript{[182]}`{=latex}
...
That is the audit the codification story requires.`\textsuperscript{[183]}`{=latex}
...
**[106] `mendeleev-1869-table`.**  **Short:** [TBD: Citation+Context]
...
**[107] `path-c-corpus-attested-valency`.**  **Short:** [TBD: Mini-essay]
...
**[108] `path-c-corpus-attested-valency`.**  **Short:** [TBD: Mini-essay]

**Deployments:** Chapter 11 §11.2 ¶ — methodology disclosure for the Path C operational definition; Chapter 11 §11.3 ¶ — anchors the 67.6% tier-share number.

Path C operationalizes a *dhātu*'s reactivity as **corpus-attested combinatorial valency** — the count of distinct (*upasarga*, *pratyaya-class*) pairs that produce attested forms in the reference Sanskrit corpus. The reference corpus is the Digital Corpus of Sanskrit (Hellwig, 2010–2024; CC BY 4.0), accessed through the `OliverHellwig/sanskrit` GitHub mirror at the 2026-05-18 commit; 15,900 CoNLL-U files; 1,007,361 VERB-tagged tokens; 3,839 unique bare *dhātavaḥ* attested across the corpus. *Pratyaya*-class is normalized as a coarse approximation of the Pāṇinian *pratyaya* space — finite forms as the tuple (Tense, Mood, Voice) e.g. *fin:Pres+Ind+Act*; non-finite forms as the VerbForm value e.g. *nfin:Part*. Path A correlation (matched MW subset of 121 *dhātavaḥ*): Spearman ρ = +0.6647. The full reproducibility bundle — scripts, derived data, methodology, sensitivity tests — lives in `analysis/ganah/` with synthesis in `FINDINGS.md`. See dossier for sub-corpus extraction details, the Bhagavadgītā-not-in-DCS substitution note, and the Rāmāyaṇa-as-smriti-epic decision.

**[109] `varga-column-as-engineering-axis`.**  **Short:** [TBD: Mini-essay]
...
**[110] `cross-gana-column-distribution`.**  **Short:** [TBD: Mini-essay]
...
**[111] `inherent-vowel-secondary-axis`.**  **Short:** [TBD: Mini-essay]
...
**[112] `cross-corpus-invariance`.**  **Short:** [TBD: Mini-essay]
...
**[145] `speculation-theory-asuric-certainty`.**  The chapter is not attacking speculation. It is attacking the asuric conversion of speculation into authorized certainty: one civilization's conjecture becomes "theory," while the civilization under study has its categories demoted to "belief."

**[146] `pre-pie-dictionary-shift`.**  The displacement of Sanskrit from the terminus of Indo-European etymological chains to one daughter alongside others (with PIE inserted as reconstructed terminus) is a two-century arc — Bopp 1816 (Sanskrit as ancestor) `{\symbolfont →}`{=latex} Schleicher 1861 (common source distinct from Sanskrit) `{\symbolfont →}`{=latex} mid-20c (academic references) `{\symbolfont →}`{=latex} 2000s (popular references); *Merriam-Webster's Collegiate 10th* (1993) still showed Sanskrit-at-terminus for *mother*. See Expanded Endnotes for the staged chronology.
...
**[175] `schleicher-1861-compendium`.**  ***August Schleicher*** (1821–1868), ***Compendium der vergleichenden Grammatik der indogermanischen Sprachen*** (Hermann Böhlau, Weimar, 1861; 2nd ed. 1866) — *the inversion moment*. The work introduces the ***Stammbaumtheorie*** (family-tree theory) explicitly: a single common ancestor *distinct from any attested language* (the *Ursprache*) branching into daughter Indo-European languages; Sanskrit, previously the anchor, is demoted to *one daughter language among siblings*. The asterisk-before-reconstructed-form convention — Schleicher's notational invention — gives the machinery a typographic mark for *forms posited but not attested*. Schleicher's 1863 pamphlet *Die Darwinsche Theorie und die Sprachwissenschaft* made the biological-organic framing explicit. *The bake* operating at its central moment.

**[176] `schleicher-1868-fable`.**  August Schleicher, "Eine fabel in indogermanischer ursprache," *Beiträge zur vergleichenden Sprachforschung* 5 (1868): 206–208 — the first complete text composed entirely in reconstructed PIE (*Avis akvāsas ka*), every word marked with the asterisk convention Schleicher had introduced in his 1861 *Compendium*; the fable has been rewritten by Hirt (1939), Lehmann-Zgusta (1979), and Kortlandt (2007) on incompatible reconstructions.

**[177] `brugmann-grundriss-1886`.**  ***Karl Brugmann*** (1849–1919), of the Leipzig ***Junggrammatiker*** (Neogrammarian) school, ***Grundriss der vergleichenden Grammatik der indogermanischen Sprachen*** (Karl J. Trübner, Strasbourg, 1886–1893; revised 1897–1916, with Berthold Delbrück's syntactic volumes) — *the cementing moment*. The work systematized the comparative method around the ***Ausnahmslosigkeit der Lautgesetze*** doctrine (sound laws operate without exception, licensing reverse-engineering) and consolidated a comprehensive reconstructed PIE — phonology, morphology, lexicon — on the basis of comparative-method work. The reconstructed ecosystem operated as a self-validating system in which methodology and output were mutually reinforcing. *The bake's industrial-scale production line*.

**[178] `brahmi-devanagari-structural-identity`.**  Brāhmī (`{\devanagarifont ब्राह्मी}`{=latex}) and Devanāgarī (`{\devanagarifont देवनागरी}`{=latex}) are structurally identical at the encoding-system level (abugida structure, *varṇamālā* (`{\devanagarifont वर्णमाला}`{=latex}) inventory, *sthāna* (`{\devanagarifont स्थान}`{=latex}) / *prayatna* (`{\devanagarifont प्रयत्न}`{=latex}) organization, conjunct formation) and differ only at the surface (glyph shapes, *śirorekhā* (`{\devanagarifont शिरोरेखा}`{=latex}), stroke ductus, numerals); the architecture both Indic scripts share has no equivalent in Aramaic, so glyph-shape borrowing cannot produce the engineered specification.

**[179] `productivity-inversion-natural-language`.**  Natural languages display the *frequency-irregularity correlation* — high-frequency forms tend toward suppletive idiosyncrasy (English *be / have / do*; Latin *esse / ire / ferre*; Greek *eimi / oida / ph`{\symbolfont ē}`{=latex}mi*); the correlation is one of the most-replicated findings in natural-language morphology, explained by high-frequency forms being mastered as wholes and resisting analogical regularization. Sanskrit shows the *opposite* pattern: the most-productive *dhātus* (*kṛ, bhū, gam, sthā, dā, dhā, jñā, hṛ, nī*) are the most structurally *minimal* (CV / CVC) *and* the most paradigmatically *regular*. Empirical signature in the *Dhātupāṭha* curated sample: Spearman ρ = −0.485 between productivity and particle count; CV pattern mean productivity 2.9× the CCVCC pattern's; top-20 productivity ranks dominated by minimal-particle patterns. Both axes are engineered at once.

**[180] `vedic-classical-circular-dating`.**  Appendix Part 7 names the circular method behind the drift story: linguistic features are used to arrange Sanskrit texts into an assumed archaic-to-later sequence, and the resulting sequence is then cited as proof that Sanskrit drifted from Vedic to Classical.

**[181] `panini-cites-pre-paninian-grammarians`.**  Pāṇini's *Aṣṭādhyāyī* (`{\devanagarifont अष्टाध्यायी}`{=latex}) cites by name a roster of pre-Pāṇinian grammarians — ***Āpiśali*** (`{\devanagarifont आपिशलि}`{=latex}, 6.1.92), ***Kāśyapa*** (`{\devanagarifont काश्यप}`{=latex}, 1.2.25, 8.4.67), ***Gārgya*** (`{\devanagarifont गार्ग्य}`{=latex}, 7.3.99, 8.3.20), ***Gālava*** (`{\devanagarifont गालव}`{=latex}, 6.3.61, 7.1.74, 8.4.67), ***Cākravarmaṇa*** (`{\devanagarifont चाक्रवर्मण}`{=latex}, 6.1.130), ***Bhāradvāja*** (`{\devanagarifont भारद्वाज}`{=latex}, 7.2.63), ***Saunaga*** (`{\devanagarifont सौनाग}`{=latex}), ***Senaka*** (`{\devanagarifont सेनक}`{=latex}, 5.4.112), and ***Sphoṭāyana*** (`{\devanagarifont स्फोटायन}`{=latex}, 6.1.123) — each citation recording where Pāṇini ratifies, modifies, or preserves alternative analyses; the *Aṣṭādhyāyī* is not the founding moment of Sanskrit grammar but the finest decoding of a discipline with substantial prior depth.

**[182] `siddhe-shabdarthasambandhe-mbh`.**  Patañjali's *Mahābhāṣya* (`{\devanagarifont महाभाष्य}`{=latex}) opens with ***siddhe śabdārthasambandhe*** (`{\devanagarifont सिद्धे}`{=latex} `{\devanagarifont शब्दार्थसम्बन्धे}`{=latex}) inside the fuller formulation ***siddhe śabdārthasambandhe lokato 'rthaprayukte śabdaprayoge śāstreṇa dharmaniyamaḥ*** (`{\devanagarifont सिद्धे}`{=latex} `{\devanagarifont शब्दार्थसम्बन्धे}`{=latex} `{\devanagarifont लोकतोऽर्थप्रयुक्ते}`{=latex} `{\devanagarifont शब्दप्रयोगे}`{=latex} `{\devanagarifont शास्त्रेण}`{=latex} `{\devanagarifont धर्मनियमः}`{=latex}). The sequence anchors Chapter 4's argument: the word-meaning bond is *siddha* (`{\devanagarifont सिद्ध}`{=latex}, already established), worldly usage follows, and *śāstra* regulates correct usage. It does not manufacture the bond. See endnote `patanjali-siddhe-shabdarthasambandhe` for the full Kielhorn-edition citation and the parallel Preface deployment.

**[183] `calibration-audit-gap`.**  The codification story requires an empirical audit it has not supplied: pre-Pāṇinian and para-Pāṇinian Sanskrit witnesses should be tested against Pāṇini's documented architecture after separating mode, meter, recension, optionality, domain, and actual drift.

**[184] `mitanni-indic-technical-vocabulary`.**  Mitanni Indic vocabulary matters here only as an external stability anchor: the attested forms look like technical transmission from an already functioning Indic system, not like a language waiting to be stabilized by later Pāṇinian codification.
