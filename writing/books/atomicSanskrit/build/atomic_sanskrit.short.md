...
The epigraph gives the engineering rule before physics does. It describes a *sūtra* as: few-syllabled, unambiguous, essence-bearing, facing in every direction, without padding, and faultless. This is the essence of **`{\devanagarifont सूत्रलाघव}`{=latex} (*sūtra-lāghava*)**: engineered brevity — maximum recoverable structure in minimum form.
...
**Distinguishability.** Compression and distinguishability pull in opposite directions. Compression wants the dhātuḥ to be small. Distinguishability wants each dhātuḥ to remain clear enough that it does not collapse into another one. A language cannot carry meaning if its shortest forms begin to sound alike.
...
The *varṇamālā* (`{\devanagarifont वर्णमाला}`{=latex}) is the sonomer inventory. The system built from those sonomers is the **`{\devanagarifont धातुः}`{=latex} (*dhātuḥ*)**.
...
The first measurement is sonomer count: the number of *varṇāḥ* inside each *dhātuḥ*. The examples already show the scale. **`{\devanagarifont कृ}`{=latex} (*kṛ*)** has two sonomers. **`{\devanagarifont गम्}`{=latex} (*gam*)** has three. **`{\devanagarifont भू}`{=latex} (*bhū*)** has two. **`{\devanagarifont दृश्}`{=latex} (*dṛś*)** has three. **`{\devanagarifont ज्ञा}`{=latex} (*jñā*)** has two.
...
The distribution lands the prediction. The 2-*mātrā* envelope alone carries almost half the inventory: **998 entries, or 46.0%**. Add the 2½-*mātrā* atoms and the coverage rises past **78%**. By 3 *mātrās*, it reaches **94%**.
...
The next step is abstraction. A **`{\devanagarifont धातुरचना}`{=latex} (*dhāturacanā*)** is the measured slot-pattern underneath a *dhātuḥ* — the atomic scaffold. Across the 2,168 *dhātavaḥ* of the *Dhātupāṭha*, many atoms share the same scaffold.
...
Different *dhātavaḥ* fill the same scaffold. **`{\devanagarifont गम्}`{=latex} (*gam*), `{\devanagarifont नम्}`{=latex} (*nam*), `{\devanagarifont पच्}`{=latex} (*pac*), and `{\devanagarifont वद्}`{=latex} (*vad*)** are four distinct atoms inhabiting one shape: consonantal contact (**C**), short-vowel nucleus (**V1**), consonantal contact (**C**) — **CV1C** <img src="figures/icons/scaffold_cv1c_gray.svg" style="height:1em;vertical-align:-0.2em" alt="CV1C scaffold">. In Pāṇini's *-ādi* naming convention (*gam*-and-following), this is the **`{\devanagarifont गमादि}`{=latex} `{\devanagarifont रचना}`{=latex} (*gamādi racanā*)** — the *gamādi* scaffold.`\textsuperscript{[108]}`{=latex} The timing is already inside the scaffold.
...
If the top-10 dominates only the inventory and dissolves in actual use, the architecture is only a list-pattern. If it dominates all four, the architecture is real.
...
- Particle count peaks at three, then drops sharply past four.
...
Distinguishability lands at the scaffold level. The cleanest test compares scaffolds inside the same timing budget. If duration alone explained the inventory, then two shapes with the same *mātrā* value should behave roughly alike. They do not.`\textsuperscript{[112]}`{=latex}
...
Compression creates the envelope. Distinguishability chooses the scaffold inside it. The full consonant-position proof belongs to Chapter 11; this section needs only the bridge: shape is non-random before individual particles are analyzed.
...
This strengthens *varṇa-vāda* rather than weakening it. The debate only makes sense inside an engineered language. A drifting natural language cannot sustain stable *varṇa-śakti* claims across its living record; its sounds shift, merge, and lose structural force. Sanskrit's sounds remain discrete, stable, composable, and analyzable. That is why the debate could exist at all.`\textsuperscript{[113]}`{=latex}

The Vedic context grounds why this matters. The *Vedas* are poems. Sound-meaning alignment is part of the verse's effect, its mantric power, its sanctity. *Mantra* (`{\devanagarifont मन्त्र}`{=latex}) itself — from the *dhātu* *man* (`{\devanagarifont मन्}`{=latex}, to think) — names the acoustic instrument that aligns sound with contemplation. Poetry is built form-first: meter, alliteration, and sonic flow drive the assembly; meaning enters that architecture as the poet works. Distinguishability is not engineered for its own sake. It is engineered so that the poetry can land.
...
The question is not whether Sanskrit has concentration and tail. Any usable language should. The question is whether the concentration and tail are architecturally governed. In Sanskrit they are. Ten *racanāḥ* carry more than 90% of the *Dhātupāṭha*'s semantic atoms. Forty-seven measured scaffolds carry the entire inventory. The tail is not loose residue; it is governed range.

*Vaicitrya* operates at three levels.

**At the *racanā* level.** The 37 long-tail scaffolds carry what the modal center cannot. Rare meanings can take rare shapes. Dense clusters can stage iconic force. Disyllabic envelopes can serve metrical contexts that demand a four-*mātrā* signature. The system permits what it does not promote.`\textsuperscript{[114]}`{=latex}

**At the morphological level.** The *chandas* mode preserves multiple infinitive endings — *-tum* (-`{\devanagarifont तुम्}`{=latex}), *-tavai* (-`{\devanagarifont तवै}`{=latex}), *-dhyai* (-`{\devanagarifont ध्यै}`{=latex}), *-tave* (-`{\devanagarifont तवे}`{=latex}), *-tos* (-`{\devanagarifont तोस्}`{=latex}), *-sani* (-`{\devanagarifont सनि}`{=latex}) — because metrical scope requires alternative syllable-counts. The *bhāṣā* mode keeps *-tum* canonical because non-metrical speech does not. The same *vaicitrya* signature one level up: range preserved where scope demands it; pruned where scope does not. Chapter 14 develops the calibration consequence.
...
The corollary's consequences run forward. Chapter 11 develops the reactive-inventory arrangement of the dhātavaḥ — atoms arranged by valency and reactivity. Chapter 12 develops the bonding chemistry that produces *śabdāḥ*. Chapter 13 develops the *Aṣṭādhyāyī* (`{\devanagarifont अष्टाध्यायी}`{=latex}) as the rule-system that operates on the entire chemistry.
...
If Sanskrit is atomic, how does the atom react?
...
It is the atom: a sound-bearing semantic unit, compact, timed, scaffolded, and capable of bonding. Chapter 10 showed how that atom is built. Chapter 11 shows how the atom becomes the first molecule.

That first molecule is the ***kriyā*** (`{\devanagarifont क्रिया}`{=latex}) — the verbal action-form. A *kriyā* is not a loose word grown from a root. It is an assembled form. The *dhātuḥ* enters an operational class, receives the class-signature, takes the verbal ending, and becomes usable speech. The atom remains recoverable inside the form.

The sonomers do not become vague. The *mātrā* discipline does not disappear. The molecule remains accountable to the particle-level precision that built the atom.
...
> ***dhātuḥ + gaṇa-operation / vikaraṇa + tiṅ-pratyaya `{\symbolfont →}`{=latex} kriyā***
>
> `{\devanagarifont धातुः}`{=latex} + `{\devanagarifont गण}`{=latex}-`{\devanagarifont क्रिया}`{=latex} / `{\devanagarifont विकरण}`{=latex} + `{\devanagarifont तिङ्}`{=latex}-`{\devanagarifont प्रत्ययः}`{=latex} `{\symbolfont →}`{=latex} `{\devanagarifont क्रिया}`{=latex}

The *Dhātupāṭha* lists the atoms. The **`{\devanagarifont गणाः}`{=latex} (*gaṇāḥ*)** are the operational classes. The **`{\devanagarifont विकरणानि}`{=latex} (*vikaraṇāni*)** are the class-signatures that put those atoms into motion. The **`{\devanagarifont तिङ्}`{=latex}-`{\devanagarifont प्रत्ययाः}`{=latex} (*tiṅ-pratyayāḥ*)** are the verbal endings that complete the action-form.

The chapter therefore follows the order §0.11 names as the book's spine: procedure first, statistical audit second. First it shows the operation: how a filled *dhātuḥ* scaffold passes through a *gaṇa*, takes a *vikaraṇa*, and becomes a *kriyā*. Only after the procedure is visible does the chapter count the distribution.

The matrix, the tiers, and the reactivity numbers are not the foundation. They are the audit.

Pāṇini did not impose this operating table. He decoded it.

## 11.2 The *Vikaraṇa* Procedure

A *gaṇaḥ* (`{\devanagarifont गणः}`{=latex}) is an operational class. A *vikaraṇa* (`{\devanagarifont विकरण}`{=latex}) is the class-signature that activates a *dhātuḥ* inside that class.`\textsuperscript{[116]}`{=latex} It is not decoration. It is the operation that prepares the atom for verbal use.

Before the table, the reader needs the operation. A filled scaffold enters a *gaṇa* operation, receives the relevant *vikaraṇa* or class-process, takes the *tiṅ-pratyaya* (`{\devanagarifont तिङ्}`{=latex}-`{\devanagarifont प्रत्ययः}`{=latex}), and becomes a *kriyā* — a verbal molecule.

> ***filled dhātuḥ `{\symbolfont →}`{=latex} gaṇa / vikaraṇa operation `{\symbolfont →}`{=latex} verbal stem `{\symbolfont →}`{=latex} kriyā***
>
> `{\devanagarifont धातुः}`{=latex} `{\symbolfont →}`{=latex} `{\devanagarifont गण}`{=latex} / `{\devanagarifont विकरण}`{=latex}-`{\devanagarifont क्रिया}`{=latex} `{\symbolfont →}`{=latex} `{\devanagarifont धात्वङ्गम्}`{=latex} `{\symbolfont →}`{=latex} `{\devanagarifont क्रिया}`{=latex}

The simple case is *bhvādi*. The atom is `{\devanagarifont पच्}`{=latex} (*pac*). The class-signature is *śap*, whose visible vowel is `{\devanagarifont अ}`{=latex} (*a*). The ending supplies `{\devanagarifont ति}`{=latex} (*ti*). The output is `{\devanagarifont पचति}`{=latex} (*pacati*).

![*Bhvādi* assembly. The dhātu `{\devanagarifont पच्}`{=latex} (*pac*) provides the three filled cells of the destination strip. The vikaraṇa `{\devanagarifont शप्}`{=latex} (*śap*) sits above with one surviving particle (`{\devanagarifont अ}`{=latex}) and two anubandhas (`{\devanagarifont श्}`{=latex}, `{\devanagarifont प्}`{=latex} — dashed, disappearing by rule). The ending `{\devanagarifont तिप्}`{=latex} (*tip*) sits above with two surviving particles (`{\devanagarifont त}`{=latex}, `{\devanagarifont इ}`{=latex}) and one anubandha (`{\devanagarifont प्}`{=latex}). Arrows trace each surviving source-particle into its empty destination slot; the lower strip shows the assembled `{\devanagarifont पचति}`{=latex} (*pacati*).](figures/build/building_kriya_pacati_assembly.svg){#fig:building-kriya-pacati-assembly width=50%}

The diagram carries forward the hexagonal vocabulary from Chapter 10, but now it also tracks where each piece comes from. Light gray marks inherited *dhātuḥ* material. Medium gray marks *vikaraṇa* material, or a change produced by the *vikaraṇa* operation. Dark gray marks the *tiṅ* ending.

The rails keep the assembly readable. The lower rail holds the original *dhātuḥ* vowel when it survives unchanged. The upper rail holds introduced or transformed vowels. Consonants stay on the consonantal rail. Adjacent consonants merge into a single hexagon, with provenance colors still visible inside the cell. The *mātrā* line below the per-*gaṇa* diagrams keeps the timing visible.

That convention matters. It prevents the finished form from becoming a blur. In `{\devanagarifont पचति}`{=latex} (*pacati*), the atom `{\devanagarifont पच्}`{=latex} (*pac*) remains recoverable. The *bhvādi* operation adds its class-signature. The ending completes the verbal molecule.

The atom has not dissolved into a word. **It has reacted**.

The same graphic grammar handles more than the default case. The next figures show how *dhātavaḥ* from other *gaṇāḥ* pass through their own operations.

`{\devanagarifont अदादि}`{=latex} (*adādi*) can operate with zero visible class-material: `{\devanagarifont अस्}`{=latex} (*as*) becomes `{\devanagarifont अस्ति}`{=latex} (*asti*) by direct bonding with the ending.

![*Adādi*. `{\devanagarifont अस्}`{=latex} (*as*) `{\symbolfont →}`{=latex} `{\devanagarifont अस्ति}`{=latex} (*asti*). Zero *vikaraṇa* surface material; the ending attaches directly. The *s-t* consonant cluster merges in the final form, with the dhātu's *s* (light gray) and the ending's *t* (dark gray) inside the same hexagon.](figures/building_kriya/output/vikarana_adadi.svg){#fig:building-kriya-vikarana-adadi width=50%}

`{\devanagarifont जुहोत्यादि}`{=latex} (*juhotyādi*) uses reduplication: `{\devanagarifont हु}`{=latex} (*hu*) becomes `{\devanagarifont जुहोति}`{=latex} (*juhoti*).

![*Juhotyādi*. `{\devanagarifont हु}`{=latex} (*hu*) `{\symbolfont →}`{=latex} `{\devanagarifont जुहोति}`{=latex} (*juhoti*). Reduplication produces `{\devanagarifont ज्}`{=latex}-`{\devanagarifont उ}`{=latex} (medium gray) before the dhātu; the dhātu's *u* is replaced by guṇa-*o* on the upper rail.](figures/building_kriya/output/vikarana_juhotyadi.svg){#fig:building-kriya-vikarana-juhotyadi width=95%}

`{\devanagarifont दिवादि}`{=latex} (*divādi*) uses the *śyan* / `{\devanagarifont य}`{=latex} (*ya*) operation: `{\devanagarifont दिव्}`{=latex} (*div*) becomes `{\devanagarifont दीव्यति}`{=latex} (*dīvyati*).

![*Divādi*. `{\devanagarifont दिव्}`{=latex} (*div*) `{\symbolfont →}`{=latex} `{\devanagarifont दीव्यति}`{=latex} (*dīvyati*). The *śyan* / *ya* operation. The dhātu's *i* lengthens to ī (vikaraṇa-induced transformation, medium gray on the upper rail). The *v-y* consonant cluster merges in the final form.](figures/building_kriya/output/vikarana_divadi.svg){#fig:building-kriya-vikarana-divadi width=90%}

`{\devanagarifont तनादि}`{=latex} (*tanādi*) shows the compact high-reactivity atom `{\devanagarifont कृ}`{=latex} (*kṛ*) entering the *u/o* operation and becoming `{\devanagarifont करोति}`{=latex} (*karoti*).

![*Tanādi*. `{\devanagarifont कृ}`{=latex} (*kṛ*) `{\symbolfont →}`{=latex} `{\devanagarifont करोति}`{=latex} (*karoti*). The *u/o* operation. The dhātu's *ṛ* splits via guṇa into *a* (upper rail, medium gray) + *r* (consonant, medium gray); the vikaraṇa adds *o* on the upper rail.](figures/building_kriya/output/vikarana_tanadi.svg){#fig:building-kriya-vikarana-tanadi width=85%}

`{\devanagarifont चुरादि}`{=latex} (*curādi*) uses the productive *ṇic* / `{\devanagarifont अय}`{=latex} (*aya*) operation: `{\devanagarifont चुर्}`{=latex} (*cur*) becomes `{\devanagarifont चोरयति}`{=latex} (*corayati*).

![*Curādi*. `{\devanagarifont चुर्}`{=latex} (*cur*) `{\symbolfont →}`{=latex} `{\devanagarifont चोरयति}`{=latex} (*corayati*). The *ṇic* / *aya* operation. The dhātu's *u* guṇas to *o* (medium gray, upper rail); the *aya* surface arises from the vikaraṇa's *i* through guṇa and sandhi.](figures/building_kriya/output/vikarana_curadi.svg){#fig:building-kriya-vikarana-curadi width=80%}

The operations differ, but the principle does not. A *dhātuḥ* enters an operational class. The class marks how the atom may be activated. The ending completes the verbal form.

Sometimes the operation adds visible material. Sometimes the operation is zero. Sometimes it reduplicates. Sometimes it changes the atom itself through guṇa, lengthening, or another surface effect. In every case, the finished *kriyā* remains an analyzable assembly.
...
The *gaṇaḥ* names the operational class. The *vikaraṇa* performs the operation.[VERIFY: present-stem derivations — verify each example against standard present-stem derivation, especially *cur `{\symbolfont →}`{=latex} corayati*, *div `{\symbolfont →}`{=latex} dīvyati*, *rudh `{\symbolfont →}`{=latex} ruṇaddhi*, *krī `{\symbolfont →}`{=latex} krīṇāti*]
...
After *anubandha* stripping, the 2,168-entry *Dhātupāṭha* inhabits 47 observed *racanā* scaffolds. The top ten scaffolds carry 1,973 entries — **91.01%** of the inventory. Those ten scaffolds do not distribute randomly across the ten *gaṇāḥ*.`\textsuperscript{[117]}`{=latex}
...
The working record here is the Digital Corpus of Sanskrit: 15,900 parsed Sanskrit files, more than a million verb-form occurrences, and 3,839 distinct verbal atoms visible in use.`\textsuperscript{[118]}`{=latex} The test does not ask whether a *dhātuḥ* is important in theory. It asks how many ways Sanskrit actually uses it.
...
Sanskrit is different because its concentration is coupled to architecture.`\textsuperscript{[120]}`{=latex}
...
The difference is not concentration alone. It is concentration plus compactness, concentration plus regular bonding, concentration plus scaffold order, concentration plus cross-domain stability. Path C gives Spearman ρ = **-0.4334** between valency and particle count; the matched dictionary subset gives **-0.490**.
...
The current periodic-axes figure places *dhātavaḥ* visible in the corpus by initial *varga* column and inherent vowel. Marker size and color encode Path C reactivity tier; the canonical nine are labeled.`\textsuperscript{[121]}`{=latex}
...
**Sanskrit was engineered. Encoded in the Vedas. Decoded by many. Pāṇini's decoding is the finest.** The *Dhātupāṭha* he decoded is Sanskrit's table of reactive atoms. Mendeleev gave chemistry its periodic table in 1869.`\textsuperscript{[125]}`{=latex} Sanskrit's has been operating for thousands of years.
...
The named *Śikṣā* texts are compact because they are training manuals, not survey essays. The *Pāṇinīya-Śikṣā*, *Yājñavalkya-Śikṣā*, *Vāsiṣṭhī-Śikṣā*, *Āpiśali-Śikṣā*, and *Bhāradvāja-Śikṣā* belong to that world of disciplined production.`\textsuperscript{[132]}`{=latex} They read like operating instructions for a precision instrument. Points of articulation are enumerated. Vowel durations are quantified in **`{\devanagarifont मात्रा}`{=latex} (*mātrā*)** counts; consonants are fixed at half a *mātrā* — ***`{\devanagarifont अर्धमात्रा}`{=latex} `{\devanagarifont तु}`{=latex} `{\devanagarifont व्यञ्जनम्}`{=latex}***.`\textsuperscript{[133]}`{=latex} Modes of contact between articulators are specified. Consequences of slippage at each point are tabulated. The mechanical character of the texts is the mark of the architecture working through them.
...
This is why the Veda is not a written text first and a recitation second. The recitation is the primary body. Writing is a later reflection. The standard enumeration of the *Vedāṅgas* preserves the priority correctly: *Śikṣā* stands first.`\textsuperscript{[134]}`{=latex} Sound-production precedes the grammar that analyzes the form, the etymology that explains it, the ritual that employs it, and the calendrical discipline that times its use.

Auditure is the foundation. *Śikṣā* trains the body that carries it.

## 15.2 The Eleven *Pāṭhas*

The eleven *pāṭhas* divide into two groups. The five *prakṛti-pāṭhas* are primary recitations. The six *vikṛti-pāṭhas* are modified recitations that apply deeper permutations. Together they form one of the densest preservation codes any civilization has produced.`\textsuperscript{[135]}`{=latex}
...
Vedic recitation survives in multiple geographically separated lineages: the Nambūdiri Brahmins of Kerala; Maharashtra Brahmin lineages; Tamil Nadu and Karnataka lineages; the Banaras and Allahabad lineages of the northern plains; Gujarat and Rajasthan lineages of the western coast; the Kashmir Pandit lineages preserved through displacement.`\textsuperscript{[139]}`{=latex} These communities did not operate as one centralized institution and have no documented contact across most of the cross-lineage matrix. They preserved through parallel *guru-shishya* chains.

The recitations can be compared. Recordings exist. Frits Staal's fieldwork on Nambūdiri recitation in the 1970s placed recitation traditions into audio and film archives.`\textsuperscript{[140]}`{=latex} Later recordings extend the corpus. Where lineages differ, the differences are named, located, and governed by *śākhā* specification. They are not random drift.

The phonetic constants match at the level the architecture predicts: *varṇa* inventory, accent where preserved, *sandhi* execution, metrical structure, and recitational discipline. The lineages become independent witnesses. Their testimony agrees.`\textsuperscript{[141]}`{=latex}

UNESCO recognized Vedic chanting in 2003 as a Masterpiece of the Oral and Intangible Heritage of Humanity.`\textsuperscript{[142]}`{=latex} The recognition is not the authority. The recitation is the authority. But the citation matters because even an external institution had to acknowledge the continuity and precision of the practice in front of it.
...
> *ṛṭuraṣāṇāṃ mūrdhā.*
>
> `\hfill`{=latex}*— Pāṇinīya Śikṣā tradition*`\textsuperscript{[145]}`{=latex}
...
The priests of progress have been writing this story for seventy years.`\textsuperscript{[146]}`{=latex} One scholar made the retroflex the seminal areal-feature exhibit. Another carried the position into the standard textbook account. A third pushed a Munda-substrate variant when the Dravidian version came under empirical strain. A fourth, from inside the framework, argued that the substrate hypothesis was not supported by the data. The dissent was filed and ignored. The textbook line continued.
...
The chapter's epigraph states the site directly. The meaning is simple: `{\devanagarifont मूर्धा}`{=latex} (*mūrdhā*) — the head / roof of the mouth — is the articulatory site for `{\devanagarifont ऋ}`{=latex} (*ṛ*), the `{\devanagarifont टु}`{=latex} (*ṭu*) class, `{\devanagarifont र}`{=latex} (*ra*), and `{\devanagarifont ष}`{=latex} (*ṣa*).`\textsuperscript{[148]}`{=latex} But the line does more than state the rule. Its own sound-body performs it. In the operative word `{\devanagarifont ऋटुरषाणां}`{=latex} (*ṛṭuraṣāṇāṃ*), the first vowel is `{\devanagarifont ऋ}`{=latex} (*ṛ*); the consonantal spine is `{\devanagarifont ट}`{=latex}-`{\devanagarifont र}`{=latex}-`{\devanagarifont ष}`{=latex}-`{\devanagarifont ण}`{=latex} (*ṭ-r-ṣ-ṇ*) — retroflex, semi-retroflex, retroflex, retroflex. The mouth must enter the site to say the rule that names the site. Sanskrit's phonetic science does not merely describe the architecture from outside. It makes the speaker perform the architecture while naming it.
...
Sanātan's own primary-source authority shows the opposite of what AIT claims. The **`{\devanagarifont असलायनसुत्त}`{=latex} (*Assalāyana Sutta*)** of the **`{\devanagarifont मज्झिमनिकाय}`{=latex} (*Majjhima Nikāya*)** preserves a dialogue in which the Buddha, responding to Assalāyana on the question of *varṇa* (`{\devanagarifont वर्ण}`{=latex}), observes that the bordering nations of **`{\devanagarifont योन}`{=latex} (*Yona*)** and **`{\devanagarifont कम्बोज}`{=latex} (*Kamboja*)** — the Greek and Iranian-frontier regions immediately outside the dharmic subcontinent — operate with only two varnas, *ārya* and *dāsa*, where the dharmic-Indic frame has the full fourfold *varṇa* system.`\textsuperscript{[154]}`{=latex}
...
That was the force of the moment. Savarkar did not need to utter the word; the audience completed it for him. The monitoring machinery of the British colonial state could not charge the speaker for a word the speaker had not spoken.`\textsuperscript{[155]}`{=latex}`\textsuperscript{[156]}`{=latex}
...
The origin question still admits two speculations. Both begin from the same epistemic position: nobody knows what is upstream of Sanskrit. One speculation admits this. The other hides it.`\textsuperscript{[158]}`{=latex}

Speculation is not the fault. Every human mind speculates at the edge of what it can know. The fault begins when speculation is laundered into certainty, when one civilization's conjecture is called theory and another civilization's self-understanding is demoted to belief.`\textsuperscript{[159]}`{=latex} Where measurement can decide, theory earns its name. Where measurement cannot reach, humility is the only honest register. The asuric move is to claim perfect knowledge precisely where perfect knowledge is unavailable.
...
> akin to Old High German *muoter*, Latin *māter*, Greek *m`{\symbolfont ē}`{=latex}t`{\symbolfont ē}`{=latex}r*, Sanskrit *mātṛ*`\textsuperscript{[160]}`{=latex}
...
And in early-19th-century philology — across the foundational works of the comparative-method machinery as it was being built`\textsuperscript{[165]}`{=latex} — Sanskrit was routinely placed at or near the source position of Indo-European etymological chains.
...
The orthodox account has a stock deflection for the *mother* case: the "nursery word" argument — the universal infant-babble cluster of *mama*-type forms across unrelated languages (Roman Jakobson, "Why 'Mama' and 'Papa'?", 1959), treated as a phonetic universal rather than evidence of cognation.`\textsuperscript{[167]}`{=latex}
...
The standard account (Mayrhofer, *Etymologisches Wörterbuch des Altindoarischen*) pushes Sanskrit *asura-* through Indo-Iranian **\*asura-** toward a *contested* PIE **\*h`{\symbolfont ₂}`{=latex}nsu-** meaning "life force" or "lord."`\textsuperscript{[170]}`{=latex}
...
> *"The root is suppletive — although `{\devanagarifont पश्यति}`{=latex} (paśyati) derives from Proto-Indo-European \*spe`{\symbolfont ḱ}`{=latex}-, the remainder of the morphological paradigm is derived from Proto-Indo-European \*der`{\symbolfont ḱ}`{=latex}-."*`\textsuperscript{[171]}`{=latex}
...
Old Persian supplies the contrast. The Behistun inscription, transliterated into Devanagari, can be read today by a fluent student of Sanskrit.`\textsuperscript{[174]}`{=latex} The grammar, the dhātu structures, much of the vocabulary — all of it sits within recognizable distance of the language a modern Indian student is trained in. A fluent modern speaker of Persian, presented with the same text, does not have the same experience. The two languages were close kin in their attested forms. They have ended up in radically different relationships to their own past.
...
The calibrant framework has three phases.`\textsuperscript{[180]}`{=latex}
...
The dates are the spine. **Franz Bopp** (1791–1867), trained at Paris under Antoine-Léonard de Chézy and at London under Henry Thomas Colebrooke, published *Über das Conjugationssystem der Sanskritsprache in Vergleichung mit jenem der griechischen, lateinischen, persischen und germanischen Sprache* in 1816.`\textsuperscript{[188]}`{=latex} The work treats Sanskrit verbal morphology as the structural anchor against which Greek, Latin, Persian, and Germanic verbal systems are compared. Bopp's *Vergleichende Grammatik der Sanskrit-, Send-, Armenischen-, Griechischen-, Lateinischen-, Litauischen-, Altslavischen-, Gothischen- und Deutschen* (1833–1852) carried the comparison across the family. Sanskrit was still the anchor; the comparative method was being built around it.
...
Brāhmī implements the same engineered architecture as Devanāgarī: the same *varṇamālā*, the same precise encoding of sonomers, with different glyph shapes.`\textsuperscript{[192]}`{=latex} The *varga* matrix gives five rows by place of articulation: velar, palatal, retroflex, dental, labial. Each row carries five manners of articulation: unvoiced unaspirated, unvoiced aspirated, voiced unaspirated, voiced aspirated, nasal. The vowel system modifies the consonant glyph by vowel value: *ā*, *i*, *ī*, *u*, *ū*, *e*, *ai*, *o*, *au*. The **`{\devanagarifont अयोगवाह}`{=latex} (*ayogavāha*)** — **`{\devanagarifont अनुस्वार}`{=latex} (*anusvāra*)** and **`{\devanagarifont विसर्ग}`{=latex} (*visarga*)** — mark breath and nasal gestures distinct from both consonants and vowels.
...
The apparatus assigns relative dates to Sanskrit texts partly through linguistic features. A text with more "archaic" features is earlier. A text with fewer "archaic" features is later. The sequence is then used to prove the language changed from archaic to later. The conclusion is folded into the method that produced the sequence.`\textsuperscript{[195]}`{=latex}
...
The bond comes first. Usage comes after. *Śāstra* regulates usage. It does not manufacture the bond.`\textsuperscript{[197]}`{=latex}
...
That is the audit the codification story requires.`\textsuperscript{[198]}`{=latex}
...
**[114] `vaicitrya-racana-tail`.**  Ch 10 §10.12 treats the non-modal *racanā* tail as *vaicitrya* — engineered range, not statistical residue. The current scaffold distribution has 47 observed *racanāḥ*: the top ten carry 1,973 of 2,168 *dhātavaḥ* (**91.01%**), leaving 195 entries across 37 additional scaffolds.
...
**[117] `racana-gana-matrix`.**  Ch 11 §11.3 cross-tabulates the current Ch 10 top-ten *racanā* scaffolds against Pāṇini's ten *gaṇāḥ*. Reproducibility script: `analysis/dhatupatha/scripts/analyze_racana_by_gana.py`; outputs: `analysis/dhatupatha/data/derived/racana_by_gana.csv` and `.md`; figure script: `figures/ganah/fig_racana_gana_matrix.py`; figure output: `figures/build/ganah_racana_gana_matrix.svg`. Current result after the anubandha/scaffold cleanup: 2,168 *Dhātupāṭha* entries, 47 observed *racanāḥ*, top ten *racanāḥ* covering 1,973 entries (**91.01%**), and 140 populated *racanā* × *gaṇa* cells out of 470 possible.

**[118] `path-c-corpus-attested-valency`.**  **Short:** [TBD: Mini-essay]
...
**[119] `dcs-vs-dhatupatha-count`.**  Ch 11 §11.7 percentages compute against the **3,839 distinct verbal atoms visible in the Digital Corpus of Sanskrit**, not the **2,168 canonical entries in the Dhātupāṭha**. The DCS surplus comes from variant readings, recension-specific entries, and atoms not registered in the canonical *Dhātupāṭha*.

**[120] `productivity-inversion-natural-language`.**  Natural languages display the *frequency-irregularity correlation* — high-frequency forms tend toward suppletive idiosyncrasy (English *be / have / do*; Latin *esse / ire / ferre*; Greek *eimi / oida / ph`{\symbolfont ē}`{=latex}mi*); the correlation is one of the most-replicated findings in natural-language morphology, explained by high-frequency forms being mastered as wholes and resisting analogical regularization. Sanskrit shows the *opposite* pattern: the most-productive *dhātus* (*kṛ, bhū, gam, sthā, dā, dhā, jñā, hṛ, nī*) are the most structurally *minimal* (CV / CVC) *and* the most paradigmatically *regular*. Empirical signature in the *Dhātupāṭha* curated sample: Spearman ρ = −0.485 between productivity and particle count; CV pattern mean productivity 2.9× the CCVCC pattern's; top-20 productivity ranks dominated by minimal-particle patterns. Both axes are engineered at once.

**[121] `varga-column-as-engineering-axis`.**  **Short:** [TBD: Mini-essay]
...
**[122] `inherent-vowel-secondary-axis`.**  **Short:** [TBD: Mini-essay]
...
**[123] `cross-gana-column-distribution`.**  **Short:** [TBD: Mini-essay]
...
**[124] `cross-corpus-invariance`.**  **Short:** [TBD: Mini-essay]
...
**[125] `mendeleev-1869-table`.**  **Short:** [TBD: Citation+Context]
...
**[159] `speculation-theory-asuric-certainty`.**  The chapter is not attacking speculation. It is attacking the asuric conversion of speculation into authorized certainty: one civilization's conjecture becomes "theory," while the civilization under study has its categories demoted to "belief."

**[160] `pre-pie-dictionary-shift`.**  The displacement of Sanskrit from the terminus of Indo-European etymological chains to one daughter alongside others (with PIE inserted as reconstructed terminus) is a two-century arc — Bopp 1816 (Sanskrit as ancestor) `{\symbolfont →}`{=latex} Schleicher 1861 (common source distinct from Sanskrit) `{\symbolfont →}`{=latex} mid-20c (academic references) `{\symbolfont →}`{=latex} 2000s (popular references); *Merriam-Webster's Collegiate 10th* (1993) still showed Sanskrit-at-terminus for *mother*. See Expanded Endnotes for the staged chronology.
...
**[191] `brugmann-grundriss-1886`.**  ***Karl Brugmann*** (1849–1919), of the Leipzig ***Junggrammatiker*** (Neogrammarian) school, ***Grundriss der vergleichenden Grammatik der indogermanischen Sprachen*** (Karl J. Trübner, Strasbourg, 1886–1893; revised 1897–1916, with Berthold Delbrück's syntactic volumes) — *the cementing moment*. The work systematized the comparative method around the ***Ausnahmslosigkeit der Lautgesetze*** doctrine (sound laws operate without exception, licensing reverse-engineering) and consolidated a comprehensive reconstructed PIE — phonology, morphology, lexicon — on the basis of comparative-method work. The reconstructed ecosystem operated as a self-validating system in which methodology and output were mutually reinforcing. *The bake's industrial-scale production line*.

**[192] `brahmi-devanagari-structural-identity`.**  Brāhmī (`{\devanagarifont ब्राह्मी}`{=latex}) and Devanāgarī (`{\devanagarifont देवनागरी}`{=latex}) are structurally identical at the encoding-system level (abugida structure, *varṇamālā* (`{\devanagarifont वर्णमाला}`{=latex}) inventory, *sthāna* (`{\devanagarifont स्थान}`{=latex}) / *prayatna* (`{\devanagarifont प्रयत्न}`{=latex}) organization, conjunct formation) and differ only at the surface (glyph shapes, *śirorekhā* (`{\devanagarifont शिरोरेखा}`{=latex}), stroke ductus, numerals); the architecture both Indic scripts share has no equivalent in Aramaic, so glyph-shape borrowing cannot produce the engineered specification.

**[193] `kaplan-zero-erasure`.**  Robert Kaplan's *The Nothing That Is: A Natural History of Zero* (Oxford University Press, 1999) is a useful named example of the popular move by which Indic mathematical engineering is displaced backward into Mesopotamia. Kaplan's telling foregrounds the Sumerian / Babylonian placeholder ancestry of zero and treats India as a later stage in the story. The book's polemic point is not that Mesopotamian placeholder practice is irrelevant. It is that a placeholder mark inside a sexagesimal scribal system is not the same achievement as the Indic decimal place-value system with zero operating as a full arithmetic object.

**[194] `productivity-inversion-natural-language`.**  Natural languages display the *frequency-irregularity correlation* — high-frequency forms tend toward suppletive idiosyncrasy (English *be / have / do*; Latin *esse / ire / ferre*; Greek *eimi / oida / ph`{\symbolfont ē}`{=latex}mi*); the correlation is one of the most-replicated findings in natural-language morphology, explained by high-frequency forms being mastered as wholes and resisting analogical regularization. Sanskrit shows the *opposite* pattern: the most-productive *dhātus* (*kṛ, bhū, gam, sthā, dā, dhā, jñā, hṛ, nī*) are the most structurally *minimal* (CV / CVC) *and* the most paradigmatically *regular*. Empirical signature in the *Dhātupāṭha* curated sample: Spearman ρ = −0.485 between productivity and particle count; CV pattern mean productivity 2.9× the CCVCC pattern's; top-20 productivity ranks dominated by minimal-particle patterns. Both axes are engineered at once.

**[195] `vedic-classical-circular-dating`.**  Appendix Part 7 names the circular method behind the drift story: linguistic features are used to arrange Sanskrit texts into an assumed archaic-to-later sequence, and the resulting sequence is then cited as proof that Sanskrit drifted from Vedic to Classical.

**[196] `panini-cites-pre-paninian-grammarians`.**  Pāṇini's *Aṣṭādhyāyī* (`{\devanagarifont अष्टाध्यायी}`{=latex}) cites by name a roster of pre-Pāṇinian grammarians — ***Āpiśali*** (`{\devanagarifont आपिशलि}`{=latex}, 6.1.92), ***Kāśyapa*** (`{\devanagarifont काश्यप}`{=latex}, 1.2.25, 8.4.67), ***Gārgya*** (`{\devanagarifont गार्ग्य}`{=latex}, 7.3.99, 8.3.20), ***Gālava*** (`{\devanagarifont गालव}`{=latex}, 6.3.61, 7.1.74, 8.4.67), ***Cākravarmaṇa*** (`{\devanagarifont चाक्रवर्मण}`{=latex}, 6.1.130), ***Bhāradvāja*** (`{\devanagarifont भारद्वाज}`{=latex}, 7.2.63), ***Saunaga*** (`{\devanagarifont सौनाग}`{=latex}), ***Senaka*** (`{\devanagarifont सेनक}`{=latex}, 5.4.112), and ***Sphoṭāyana*** (`{\devanagarifont स्फोटायन}`{=latex}, 6.1.123) — each citation recording where Pāṇini ratifies, modifies, or preserves alternative analyses; the *Aṣṭādhyāyī* is not the founding moment of Sanskrit grammar but the finest decoding of a discipline with substantial prior depth.

**[197] `siddhe-shabdarthasambandhe-mbh`.**  Patañjali's *Mahābhāṣya* (`{\devanagarifont महाभाष्य}`{=latex}) opens with ***siddhe śabdārthasambandhe*** (`{\devanagarifont सिद्धे}`{=latex} `{\devanagarifont शब्दार्थसम्बन्धे}`{=latex}) inside the fuller formulation ***siddhe śabdārthasambandhe lokato 'rthaprayukte śabdaprayoge śāstreṇa dharmaniyamaḥ*** (`{\devanagarifont सिद्धे}`{=latex} `{\devanagarifont शब्दार्थसम्बन्धे}`{=latex} `{\devanagarifont लोकतोऽर्थप्रयुक्ते}`{=latex} `{\devanagarifont शब्दप्रयोगे}`{=latex} `{\devanagarifont शास्त्रेण}`{=latex} `{\devanagarifont धर्मनियमः}`{=latex}). The sequence anchors Chapter 4's argument: the word-meaning bond is *siddha* (`{\devanagarifont सिद्ध}`{=latex}, already established), worldly usage follows, and *śāstra* regulates correct usage. It does not manufacture the bond. See endnote `patanjali-siddhe-shabdarthasambandhe` for the full Kielhorn-edition citation and the parallel Preface deployment.

**[198] `calibration-audit-gap`.**  The codification story requires an empirical audit it has not supplied: pre-Pāṇinian and para-Pāṇinian Sanskrit witnesses should be tested against Pāṇini's documented architecture after separating mode, meter, recension, optionality, domain, and actual drift.

**[199] `mitanni-indic-technical-vocabulary`.**  Mitanni Indic vocabulary matters here only as an external stability anchor: the attested forms look like technical transmission from an already functioning Indic system, not like a language waiting to be stabilized by later Pāṇinian codification.
