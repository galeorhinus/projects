...
The *paramparā* names the human-side conduit by which the corpus arrived: the ***dṛṣṭāḥ*** (`{\devanagarifont दृष्टाः}`{=latex}, *the seers*), who saw the *Vedas* the eternal *śabda* manifests as. They are ***mantra-dṛṣṭāḥ*** (`{\devanagarifont मन्त्रद्रष्टाः}`{=latex}), not ***mantra-kartṛs*** — *mantra*-seers, not *mantra*-composers; they received what they saw, they did not compose it. The *Mīmāṃsā* doctrine of ***apauruṣeyatva*** (`{\devanagarifont अपौरुषेयत्व}`{=latex}) is the *paramparā*'s own anchor for the position: no separate human agent-class is hypothesized between the eternal *śabda* and the *dṛṣṭāḥ*.`\textsuperscript{[3]}`{=latex} The grammarians who systematically documented the system — Pāṇini, Kātyāyana, Patañjali, and the pre-Pāṇinian roster — came later still. None of them invented Sanskrit. They decoded what the *Vedas* encode. In the Indic ethos, knowledge belongs to the *paramparā*, not to a named inventor. Insight is ***heard*** (*śruti*), not authored.
...
>
> *Given that the bond between word and meaning is established, and given that worldly word-usage is prompted by meaning, śāstra regulates correct usage.*`\textsuperscript{[46]}`{=latex}
...
>
> *Many are the corruptions; few are the words.*`\textsuperscript{[47]}`{=latex}
...
> **`{\devanagarifont गावी}`{=latex} (*gāvī*), `{\devanagarifont गोणी}`{=latex} (*goṇī*), `{\devanagarifont गोता}`{=latex} (*gotā*), `{\devanagarifont गोपोतलिका}`{=latex} (*gopotalikā*).**`\textsuperscript{[48]}`{=latex}
...
The orthodox account claims direct evidence of drift inside the Vedic corpus. It points to differences across the four Vedas, across Rigvedic *maṇḍalas*, across Saṃhitā / Brāhmaṇa / Āraṇyaka / Upaniṣad layers, across recensions, across accent systems, and across word forms.`\textsuperscript{[49]}`{=latex}
...
The tongue is the most complex moving part: tip, blade, body, and base. Above it sit the upper teeth, the alveolar ridge, the hard palate, the soft palate, and the uvula. At the front are the lips. At the back is the throat. The distance from lips to glottis averages roughly seventeen centimeters in adult males, ~14–15 cm in adult females, and shorter still in children, with the adult range running from about 13 cm to about 20 cm. The architecture is the same.`\textsuperscript{[56]}`{=latex}
...
That unit is the **`{\devanagarifont अक्षर}`{=latex} (*akṣara*)** — the imperishable. The morphology is exact: *a-* (privative) + √*kṣar* (to flow, to perish) — *that which does not decay*. The name is not neutral. Latin has *littera*, a mark. Greek has *gramma*, what is written. Arabic has *ḥarf*, an edge. Sanskrit names the writing-and-utterance primitive *akṣara*, the imperishable. The non-decay claim sits inside the name.`\textsuperscript{[72]}`{=latex}
...
The vocabulary developed here is not modern explanation imposed backward. *Sthāna*, *karaṇa*, *prayatna*, *prāṇa*, *ghoṣa*, *anunāsika*, *sparśa*, *swara*, *antaḥstha*, *ūṣman*, *varga*, *varṇa*, and *varṇamālā* belong to the Sanskrit grammatical and phonetic disciplines themselves. The *Prātiśākhya* and *Śikṣā* materials document the architecture as already operating vocabulary.`\textsuperscript{[73]}`{=latex}`\textsuperscript{[74]}`{=latex}
...
Frits Staal saw the structural point clearly when he compared the *varga* system to Mendeleev's periodic table.`\textsuperscript{[75]}`{=latex} The comparison is right. Each *sparśa* consonant sits at a coordinate determined by constituent properties, just as a chemical element sits inside a reference system determined by its constituent properties.
...
The same logic extends beyond *sparśa*. *Swaras* sample vowel-space and add time: *hrasva*, *dīrgha*, and *pluta*.`\textsuperscript{[81]}`{=latex} *Antaḥstha* occupy the in-between mode. *Ūṣman* operate constricted breath. *Ayogavāha* specifies breath termination. The full *varṇamālā* is not a list. It is the integrated specification of the speaking body.
...
The southern languages — Tamil, Kannada, Malayalam, Telugu, Tulu — preserve the five-zone place axis and the retroflex series. They do not operate the full *mahāprāṇa* series natively in the same way Sanskrit does; aspirates generally enter through Sanskritic vocabulary.`\textsuperscript{[84]}`{=latex} That difference matters. These languages are not failed Sanskrit. They are parallel selections from the same regional substrate.
...
The eastern subcontinent — Bengali, Odia, Assamese, Maithili's eastern edge — preserves the retroflex series and the five-zone axis. Bengali's `{\devanagarifont व}`{=latex} / `{\devanagarifont ब}`{=latex} merger occurs inside the labial zone, exactly where the matrix carries a close neighbor. Odia remains especially conservative in its consonant inventory.`\textsuperscript{[85]}`{=latex}

The Sindhu region adds the cleanest exclusion case. Sindhi preserves retroflexion, the five-zone axis, and aspiration, but also operates implosives — ɓ, ɗ, ʄ, ɠ — that Sanskrit does not include.`\textsuperscript{[86]}`{=latex} The sound-field contains them. The *varṇamālā* rejects them.
...
Now compare the matrix with the broader superset. The subcontinental field contains more than the five Sanskrit places. Tamil operates an alveolar contact-station distinct from dental and retroflex.`\textsuperscript{[87]}`{=latex} Ho, Mundari, and related languages operate glottal stops or checked consonants.`\textsuperscript{[88]}`{=latex} Persian-and-Arabic-influenced registers carry labio-dental fricatives such as *f*.`\textsuperscript{[89]}`{=latex} Sindhi operates implosives. Some regional systems carry fine distinctions around the palatal zone.
...
Punjabi confirms the danger. Its tones developed after voiced aspirates weakened or merged; the lost consonantal contrast leaked into neighboring vowels as pitch.`\textsuperscript{[90]}`{=latex} Some Pahari languages show related tonal developments.`\textsuperscript{[91]}`{=latex} Tone here is evidence of drift after an older consonantal contrast weakened. Sanskrit's preserved grid prevents that drift.
...
**Sindhi implosives.** Sindhi carries implosive consonants. Sanskrit does not. Implosives are voiced stops with inward airflow, acoustically close enough to ordinary voiced stops to crowd the row. The field contains them; the grid rejects them.`\textsuperscript{[92]}`{=latex}

**Tamil alveolar.** Tamil preserves an alveolar contact-station between dental and retroflex. Sanskrit excludes it. The tongue-tip axis would become too crowded if dental, alveolar, and retroflex were all full grid rows.`\textsuperscript{[93]}`{=latex}

**Glottal stop.** Some central-eastern languages use glottal closure phonemically. Sanskrit does not. It assigns the glottal/breath region to different work: *visarga* and related breath-gestures.`\textsuperscript{[94]}`{=latex}

**Labio-dental fricatives.** The *f* / *v* region appears through contact registers and in non-subcontinental languages. Sanskrit keeps the labial row bilabial. It does not crowd the front edge of the mouth with a second labial contact-station.`\textsuperscript{[95]}`{=latex}
...
The evidence is visible in the architecture. No signed plan is needed for the same reason no signed plan is needed to recognize the engineering of Kailasa at Ellora. The temple's architects are anonymous; the temple's engineering is not.`\textsuperscript{[97]}`{=latex} The rock carries the record.
...
Yaska's decoding of **`{\devanagarifont अग्नि}`{=latex} (*agni*)** at *Nirukta* 7.14 makes the point concrete. The text predates Pāṇini and is foundational to the etymological decoding discipline. Yaska treats one word with four independent *dhātu*-level decodings, two attributed to earlier named pre-Pāṇinian decoders:`\textsuperscript{[101]}`{=latex}
...
He named the ten **`{\devanagarifont गणाः}`{=latex} (*gaṇāḥ*)**, the inflectional classes. He documented the **`{\devanagarifont विकरण}`{=latex} (*vikaraṇa*)**, the column signature that distinguishes one conjugational pattern from another.`\textsuperscript{[102]}`{=latex} He showed how *dhātavaḥ* enter the grammar's molecular machinery.
...
Mendeleev organized the chemical elements by structural property; chemical behavior emerged from position in the table.`\textsuperscript{[103]}`{=latex} He did not sort elements by what they "meant." He sorted them by behavior, and the chemistry became visible.
...
*Kṛ* (`{\devanagarifont कृ}`{=latex}, to make) carries valency 1,062 in the Digital Corpus of Sanskrit.`\textsuperscript{[104]}`{=latex} *Bhū* (`{\devanagarifont भू}`{=latex}, to be) carries 504. *Dhā* (`{\devanagarifont धा}`{=latex}, to place) carries 386. These are not claims about prestige or usefulness. They are corpus-attested bonding counts.
...
The periodic-table column for an atom is the column of its first consonant.`\textsuperscript{[106]}`{=latex} Semivowels, sibilants, and `{\devanagarifont ह}`{=latex} function as additional structural columns. Vowel-initial atoms occupy a separate band.
...
This is why the Veda is not a written text first and a recitation second. The recitation is the primary body. Writing is a later reflection. The standard enumeration of the *Vedāṅgas* preserves the priority correctly: *Śikṣā* stands first.`\textsuperscript{[117]}`{=latex} Sound-production precedes the grammar that analyzes the form, the etymology that explains it, the ritual that employs it, and the calendrical discipline that times its use.

Auditure is the foundation. *Śikṣā* trains the body that carries it.

## 15.2 The Eleven *Pāṭhas*

The eleven *pāṭhas* divide into two groups. The five *prakṛti-pāṭhas* are primary recitations. The six *vikṛti-pāṭhas* are modified recitations that apply deeper permutations. Together they form one of the densest preservation codes any civilization has produced.`\textsuperscript{[118]}`{=latex}
...
Vedic recitation survives in multiple geographically separated lineages: the Nambūdiri Brahmins of Kerala; Maharashtra Brahmin lineages; Tamil Nadu and Karnataka lineages; the Banaras and Allahabad lineages of the northern plains; Gujarat and Rajasthan lineages of the western coast; the Kashmir Pandit lineages preserved through displacement.`\textsuperscript{[122]}`{=latex} These communities did not operate as one centralized institution and have no documented contact across most of the cross-lineage matrix. They preserved through parallel *guru-shishya* chains.

The recitations can be compared. Recordings exist. Frits Staal's fieldwork on Nambūdiri recitation in the 1970s placed recitation traditions into audio and film archives.`\textsuperscript{[123]}`{=latex} Later recordings extend the corpus. Where lineages differ, the differences are named, located, and governed by *śākhā* specification. They are not random drift.

The phonetic constants match at the level the architecture predicts: *varṇa* inventory, accent where preserved, *sandhi* execution, metrical structure, and recitational discipline. The lineages become independent witnesses. Their testimony agrees.`\textsuperscript{[124]}`{=latex}

UNESCO recognized Vedic chanting in 2003 as a Masterpiece of the Oral and Intangible Heritage of Humanity.`\textsuperscript{[125]}`{=latex} The recognition is not the authority. The recitation is the authority. But the citation matters because even an external institution had to acknowledge the continuity and precision of the practice in front of it.
...
The priests of progress have been writing this story for seventy years.`\textsuperscript{[128]}`{=latex} One scholar made the retroflex the seminal areal-feature exhibit. Another carried the position into the standard textbook account. A third pushed a Munda-substrate variant when the Dravidian version came under empirical strain. A fourth, from inside the framework, argued that the substrate hypothesis was not supported by the data. The dissent was filed and ignored. The textbook line continued.
...
The chapter's epigraph states the site directly. The meaning is simple: `{\devanagarifont मूर्धा}`{=latex} (*mūrdhā*) — the head / roof of the mouth — is the articulatory site for `{\devanagarifont ऋ}`{=latex} (*ṛ*), the `{\devanagarifont टु}`{=latex} (*ṭu*) class, `{\devanagarifont र}`{=latex} (*ra*), and `{\devanagarifont ष}`{=latex} (*ṣa*).`\textsuperscript{[130]}`{=latex} But the line does more than state the rule. Its own sound-body performs it. In the operative word `{\devanagarifont ऋटुरषाणां}`{=latex} (*ṛṭuraṣāṇāṃ*), the first vowel is `{\devanagarifont ऋ}`{=latex} (*ṛ*); the consonantal spine is `{\devanagarifont ट}`{=latex}-`{\devanagarifont र}`{=latex}-`{\devanagarifont ष}`{=latex}-`{\devanagarifont ण}`{=latex} (*ṭ-r-ṣ-ṇ*) — retroflex, semi-retroflex, retroflex, retroflex. The mouth must enter the site to say the rule that names the site. Sanskrit's phonetic science does not merely describe the architecture from outside. It makes the speaker perform the architecture while naming it.
...
Sanātan's own primary-source authority shows the opposite of what AIT claims. The **`{\devanagarifont असलायनसुत्त}`{=latex} (*Assalāyana Sutta*)** of the **`{\devanagarifont मज्झिमनिकाय}`{=latex} (*Majjhima Nikāya*)** preserves a dialogue in which the Buddha, responding to Assalāyana on the question of *varṇa* (`{\devanagarifont वर्ण}`{=latex}), observes that the bordering nations of **`{\devanagarifont योन}`{=latex} (*Yona*)** and **`{\devanagarifont कम्बोज}`{=latex} (*Kamboja*)** — the Greek and Iranian-frontier regions immediately outside the dharmic subcontinent — operate with only two varnas, *ārya* and *dāsa*, where the dharmic-Indic frame has the full fourfold *varṇa* system.`\textsuperscript{[136]}`{=latex}
...
That was the force of the moment. Savarkar did not need to utter the word; the audience completed it for him. The monitoring machinery of the British colonial state could not charge the speaker for a word the speaker had not spoken.`\textsuperscript{[137]}`{=latex}`\textsuperscript{[138]}`{=latex}
...
The origin question still admits two speculations. Both begin from the same epistemic position: nobody knows what is upstream of Sanskrit. One speculation admits this. The other hides it.`\textsuperscript{[140]}`{=latex}
...
> akin to Old High German *muoter*, Latin *māter*, Greek *m`{\symbolfont ē}`{=latex}t`{\symbolfont ē}`{=latex}r*, Sanskrit *mātṛ*`\textsuperscript{[141]}`{=latex}
...
And in early-19th-century philology — across the foundational works of the comparative-method machinery as it was being built`\textsuperscript{[146]}`{=latex} — Sanskrit was routinely placed at or near the source position of Indo-European etymological chains.
...
The orthodox account has a stock deflection for the *mother* case: the "nursery word" argument — the universal infant-babble cluster of *mama*-type forms across unrelated languages (Roman Jakobson, "Why 'Mama' and 'Papa'?", 1959), treated as a phonetic universal rather than evidence of cognation.`\textsuperscript{[148]}`{=latex}
...
The standard account (Mayrhofer, *Etymologisches Wörterbuch des Altindoarischen*) pushes Sanskrit *asura-* through Indo-Iranian **\*asura-** toward a *contested* PIE **\*h`{\symbolfont ₂}`{=latex}nsu-** meaning "life force" or "lord."`\textsuperscript{[151]}`{=latex}
...
> *"The root is suppletive — although `{\devanagarifont पश्यति}`{=latex} (paśyati) derives from Proto-Indo-European \*spe`{\symbolfont ḱ}`{=latex}-, the remainder of the morphological paradigm is derived from Proto-Indo-European \*der`{\symbolfont ḱ}`{=latex}-."*`\textsuperscript{[152]}`{=latex}
...
Old Persian supplies the contrast. The Behistun inscription, transliterated into Devanagari, can be read today by a fluent student of Sanskrit.`\textsuperscript{[155]}`{=latex} The grammar, the dhātu structures, much of the vocabulary — all of it sits within recognizable distance of the language a modern Indian student is trained in. A fluent modern speaker of Persian, presented with the same text, does not have the same experience. The two languages were close kin in their attested forms. They have ended up in radically different relationships to their own past.
...
The calibrant framework has three phases.`\textsuperscript{[161]}`{=latex}
...
The dates are the spine. **Franz Bopp** (1791–1867), trained at Paris under Antoine-Léonard de Chézy and at London under Henry Thomas Colebrooke, published *Über das Conjugationssystem der Sanskritsprache in Vergleichung mit jenem der griechischen, lateinischen, persischen und germanischen Sprache* in 1816.`\textsuperscript{[169]}`{=latex} The work treats Sanskrit verbal morphology as the structural anchor against which Greek, Latin, Persian, and Germanic verbal systems are compared. Bopp's *Vergleichende Grammatik der Sanskrit-, Send-, Armenischen-, Griechischen-, Lateinischen-, Litauischen-, Altslavischen-, Gothischen- und Deutschen* (1833–1852) carried the comparison across the family. Sanskrit was still the anchor; the comparative method was being built around it.
...
**[103] `mendeleev-1869-table`.**  **Short:** [TBD: Citation+Context]
...
**[104] `path-c-corpus-attested-valency`.**  **Short:** [TBD: Mini-essay]
...
**[106] `varga-column-as-engineering-axis`.**  **Short:** [TBD: Mini-essay]
...
**[107] `cross-gana-column-distribution`.**  **Short:** [TBD: Mini-essay]
...
**[108] `inherent-vowel-secondary-axis`.**  **Short:** [TBD: Mini-essay]
...
**[109] `cross-corpus-invariance`.**  **Short:** [TBD: Mini-essay]
