...
Punjabi confirms the danger. Its tones developed after voiced aspirates weakened or merged; the lost consonantal contrast leaked into neighboring vowels as pitch.`\textsuperscript{[91]}`{=latex} Some Pahari languages show related tonal developments.`\textsuperscript{[92]}`{=latex} Tone here is evidence of drift after an older consonantal contrast weakened. Sanskrit's preserved grid prevents that drift.
...
**Sindhi implosives.** Sindhi carries implosive consonants. Sanskrit does not. Implosives are voiced stops with inward airflow, acoustically close enough to ordinary voiced stops to crowd the row. The field contains them; the grid rejects them.`\textsuperscript{[93]}`{=latex}

**Tamil alveolar.** Tamil preserves an alveolar contact-station between dental and retroflex. Sanskrit excludes it. The tongue-tip axis would become too crowded if dental, alveolar, and retroflex were all full grid rows.`\textsuperscript{[94]}`{=latex}

**Glottal stop.** Some central-eastern languages use glottal closure phonemically. Sanskrit does not. It assigns the glottal/breath region to different work: *visarga* and related breath-gestures.`\textsuperscript{[95]}`{=latex}

**Labio-dental fricatives.** The *f* / *v* region appears through contact registers and in non-subcontinental languages. Sanskrit keeps the labial row bilabial. It does not crowd the front edge of the mouth with a second labial contact-station.`\textsuperscript{[96]}`{=latex}
...
The evidence is visible in the architecture. No signed plan is needed for the same reason no signed plan is needed to recognize the engineering of Kailasa at Ellora. The temple's architects are anonymous; the temple's engineering is not.`\textsuperscript{[98]}`{=latex} The rock carries the record.
...
*Kṛ* (`{\devanagarifont कृ}`{=latex}) is not a loose sound. It is one consonantal particle and one vocalic particle, arranged as a stable semantic atom: to do, to make. *Gam* (`{\devanagarifont गम्}`{=latex}) is consonant-vowel-consonant: to go. *Bhū* (`{\devanagarifont भू}`{=latex}) is consonant-vowel: to be. The order matters. The particles matter. The stability matters.
...
But compression alone fails. It cannot explain why C2 is rarer than C4, even though C4 is more articulatorily expensive. It cannot explain why the typologically rare vowel `{\devanagarifont ऋ}`{=latex} (*ṛ*) becomes critical. It cannot explain why some grid cells are heavily deployed and others remain almost empty. Compression tells the system to be small. It does not tell the system which particles to choose.
...
None of these disciplines can operate on a language drifting in the way the orthodox account describes Sanskrit. Their joint existence — running for thousands of years, across many *guru-shishya paramparā* lineages, with each discipline's analytical work cross-checking the others — is the evidence.

Yaska's decoding of **`{\devanagarifont अग्नि}`{=latex} (*agni*)** at *Nirukta* 7.14 makes the point concrete. The text predates Pāṇini and is foundational to the etymological decoding discipline. Yaska treats one word with four independent *dhātu*-level decodings, two attributed to earlier named pre-Pāṇinian decoders:`\textsuperscript{[102]}`{=latex}
...
**Forward to Ch 11 §11.1.** The matrix analyses Ch 11 runs against the *Dhātupāṭha* — place-of-articulation asymmetry tables, OCP enforcement rates, cross-corpus combinatorial yield, the periodic-table grid itself — are corrective work the *vaiyākaraṇāḥ* did not have to undertake. The *paramparā* did not need them; the orthodox denial requires them. **Sanskrit was engineered. The *vaiyākaraṇāḥ* documented. The audience verified. The orthodox account denies. The empirical re-proof is the cost of having to argue against the denial.** Ch 11 §11.1 develops the implication directly.
...
He named the ten **`{\devanagarifont गणाः}`{=latex} (*gaṇāḥ*)**, the inflectional classes. He documented the **`{\devanagarifont विकरण}`{=latex} (*vikaraṇa*)**, the column signature that distinguishes one conjugational pattern from another.`\textsuperscript{[103]}`{=latex} He showed how *dhātavaḥ* enter the grammar's molecular machinery.
...
Mendeleev organized the chemical elements by structural property; chemical behavior emerged from position in the table.`\textsuperscript{[104]}`{=latex} He did not sort elements by what they "meant." He sorted them by behavior, and the chemistry became visible.
...
*Kṛ* (`{\devanagarifont कृ}`{=latex}, to make) carries valency 1,062 in the Digital Corpus of Sanskrit.`\textsuperscript{[105]}`{=latex} *Bhū* (`{\devanagarifont भू}`{=latex}, to be) carries 504. *Dhā* (`{\devanagarifont धा}`{=latex}, to place) carries 386. These are not claims about prestige or usefulness. They are corpus-attested bonding counts.
...
The periodic-table column for an atom is the column of its first consonant.`\textsuperscript{[107]}`{=latex} Semivowels, sibilants, and `{\devanagarifont ह}`{=latex} function as additional structural columns. Vowel-initial atoms occupy a separate band.
...
The named *Śikṣā* texts are compact because they are training manuals, not survey essays. The *Pāṇinīya-Śikṣā*, *Yājñavalkya-Śikṣā*, *Vāsiṣṭhī-Śikṣā*, *Āpiśali-Śikṣā*, and *Bhāradvāja-Śikṣā* belong to that world of disciplined production.`\textsuperscript{[117]}`{=latex} They read like operating instructions for a precision instrument. Points of articulation are enumerated. Vowel durations are quantified in **`{\devanagarifont मात्रा}`{=latex} (*mātrā*)** counts; consonants are fixed at half a *mātrā* — ***`{\devanagarifont अर्धमात्रा}`{=latex} `{\devanagarifont तु}`{=latex} `{\devanagarifont व्यञ्जनम्}`{=latex}***.`\textsuperscript{[118]}`{=latex} Modes of contact between articulators are specified. Consequences of slippage at each point are tabulated. The mechanical character of the texts is the mark of the architecture working through them.
...
This is why the Veda is not a written text first and a recitation second. The recitation is the primary body. Writing is a later reflection. The standard enumeration of the *Vedāṅgas* preserves the priority correctly: *Śikṣā* stands first.`\textsuperscript{[119]}`{=latex} Sound-production precedes the grammar that analyzes the form, the etymology that explains it, the ritual that employs it, and the calendrical discipline that times its use.

Auditure is the foundation. *Śikṣā* trains the body that carries it.

## 15.2 The Eleven *Pāṭhas*

The eleven *pāṭhas* divide into two groups. The five *prakṛti-pāṭhas* are primary recitations. The six *vikṛti-pāṭhas* are modified recitations that apply deeper permutations. Together they form one of the densest preservation codes any civilization has produced.`\textsuperscript{[120]}`{=latex}
...
Vedic recitation survives in multiple geographically separated lineages: the Nambūdiri Brahmins of Kerala; Maharashtra Brahmin lineages; Tamil Nadu and Karnataka lineages; the Banaras and Allahabad lineages of the northern plains; Gujarat and Rajasthan lineages of the western coast; the Kashmir Pandit lineages preserved through displacement.`\textsuperscript{[124]}`{=latex} These communities did not operate as one centralized institution and have no documented contact across most of the cross-lineage matrix. They preserved through parallel *guru-shishya* chains.

The recitations can be compared. Recordings exist. Frits Staal's fieldwork on Nambūdiri recitation in the 1970s placed recitation traditions into audio and film archives.`\textsuperscript{[125]}`{=latex} Later recordings extend the corpus. Where lineages differ, the differences are named, located, and governed by *śākhā* specification. They are not random drift.

The phonetic constants match at the level the architecture predicts: *varṇa* inventory, accent where preserved, *sandhi* execution, metrical structure, and recitational discipline. The lineages become independent witnesses. Their testimony agrees.`\textsuperscript{[126]}`{=latex}

UNESCO recognized Vedic chanting in 2003 as a Masterpiece of the Oral and Intangible Heritage of Humanity.`\textsuperscript{[127]}`{=latex} The recognition is not the authority. The recitation is the authority. But the citation matters because even an external institution had to acknowledge the continuity and precision of the practice in front of it.
...
The priests of progress have been writing this story for seventy years.`\textsuperscript{[130]}`{=latex} One scholar made the retroflex the seminal areal-feature exhibit. Another carried the position into the standard textbook account. A third pushed a Munda-substrate variant when the Dravidian version came under empirical strain. A fourth, from inside the framework, argued that the substrate hypothesis was not supported by the data. The dissent was filed and ignored. The textbook line continued.
...
The chapter's epigraph states the site directly. The meaning is simple: `{\devanagarifont मूर्धा}`{=latex} (*mūrdhā*) — the head / roof of the mouth — is the articulatory site for `{\devanagarifont ऋ}`{=latex} (*ṛ*), the `{\devanagarifont टु}`{=latex} (*ṭu*) class, `{\devanagarifont र}`{=latex} (*ra*), and `{\devanagarifont ष}`{=latex} (*ṣa*).`\textsuperscript{[132]}`{=latex} But the line does more than state the rule. Its own sound-body performs it. In the operative word `{\devanagarifont ऋटुरषाणां}`{=latex} (*ṛṭuraṣāṇāṃ*), the first vowel is `{\devanagarifont ऋ}`{=latex} (*ṛ*); the consonantal spine is `{\devanagarifont ट}`{=latex}-`{\devanagarifont र}`{=latex}-`{\devanagarifont ष}`{=latex}-`{\devanagarifont ण}`{=latex} (*ṭ-r-ṣ-ṇ*) — retroflex, semi-retroflex, retroflex, retroflex. The mouth must enter the site to say the rule that names the site. Sanskrit's phonetic science does not merely describe the architecture from outside. It makes the speaker perform the architecture while naming it.
...
Sanātan's own primary-source authority shows the opposite of what AIT claims. The **`{\devanagarifont असलायनसुत्त}`{=latex} (*Assalāyana Sutta*)** of the **`{\devanagarifont मज्झिमनिकाय}`{=latex} (*Majjhima Nikāya*)** preserves a dialogue in which the Buddha, responding to Assalāyana on the question of *varṇa* (`{\devanagarifont वर्ण}`{=latex}), observes that the bordering nations of **`{\devanagarifont योन}`{=latex} (*Yona*)** and **`{\devanagarifont कम्बोज}`{=latex} (*Kamboja*)** — the Greek and Iranian-frontier regions immediately outside the dharmic subcontinent — operate with only two varnas, *ārya* and *dāsa*, where the dharmic-Indic frame has the full fourfold *varṇa* system.`\textsuperscript{[138]}`{=latex}
...
That was the force of the moment. Savarkar did not need to utter the word; the audience completed it for him. The monitoring machinery of the British colonial state could not charge the speaker for a word the speaker had not spoken.`\textsuperscript{[139]}`{=latex}`\textsuperscript{[140]}`{=latex}
...
The origin question still admits two speculations. Both begin from the same epistemic position: nobody knows what is upstream of Sanskrit. One speculation admits this. The other hides it.`\textsuperscript{[142]}`{=latex}

Speculation is not the fault. Every human mind speculates at the edge of what it can know. The fault begins when speculation is laundered into certainty, when one civilization's conjecture is called theory and another civilization's self-understanding is demoted to belief.`\textsuperscript{[143]}`{=latex} Where measurement can decide, theory earns its name. Where measurement cannot reach, humility is the only honest register. The asuric move is to claim perfect knowledge precisely where perfect knowledge is unavailable.
...
> akin to Old High German *muoter*, Latin *māter*, Greek *m`{\symbolfont ē}`{=latex}t`{\symbolfont ē}`{=latex}r*, Sanskrit *mātṛ*`\textsuperscript{[144]}`{=latex}
...
And in early-19th-century philology — across the foundational works of the comparative-method machinery as it was being built`\textsuperscript{[149]}`{=latex} — Sanskrit was routinely placed at or near the source position of Indo-European etymological chains.
...
The orthodox account has a stock deflection for the *mother* case: the "nursery word" argument — the universal infant-babble cluster of *mama*-type forms across unrelated languages (Roman Jakobson, "Why 'Mama' and 'Papa'?", 1959), treated as a phonetic universal rather than evidence of cognation.`\textsuperscript{[151]}`{=latex}
...
The standard account (Mayrhofer, *Etymologisches Wörterbuch des Altindoarischen*) pushes Sanskrit *asura-* through Indo-Iranian **\*asura-** toward a *contested* PIE **\*h`{\symbolfont ₂}`{=latex}nsu-** meaning "life force" or "lord."`\textsuperscript{[154]}`{=latex}
...
> *"The root is suppletive — although `{\devanagarifont पश्यति}`{=latex} (paśyati) derives from Proto-Indo-European \*spe`{\symbolfont ḱ}`{=latex}-, the remainder of the morphological paradigm is derived from Proto-Indo-European \*der`{\symbolfont ḱ}`{=latex}-."*`\textsuperscript{[155]}`{=latex}
...
Old Persian supplies the contrast. The Behistun inscription, transliterated into Devanagari, can be read today by a fluent student of Sanskrit.`\textsuperscript{[158]}`{=latex} The grammar, the dhātu structures, much of the vocabulary — all of it sits within recognizable distance of the language a modern Indian student is trained in. A fluent modern speaker of Persian, presented with the same text, does not have the same experience. The two languages were close kin in their attested forms. They have ended up in radically different relationships to their own past.
...
The calibrant framework has three phases.`\textsuperscript{[164]}`{=latex}
...
The dates are the spine. **Franz Bopp** (1791–1867), trained at Paris under Antoine-Léonard de Chézy and at London under Henry Thomas Colebrooke, published *Über das Conjugationssystem der Sanskritsprache in Vergleichung mit jenem der griechischen, lateinischen, persischen und germanischen Sprache* in 1816.`\textsuperscript{[172]}`{=latex} The work treats Sanskrit verbal morphology as the structural anchor against which Greek, Latin, Persian, and Germanic verbal systems are compared. Bopp's *Vergleichende Grammatik der Sanskrit-, Send-, Armenischen-, Griechischen-, Lateinischen-, Litauischen-, Altslavischen-, Gothischen- und Deutschen* (1833–1852) carried the comparison across the family. Sanskrit was still the anchor; the comparative method was being built around it.
...
**[104] `mendeleev-1869-table`.**  **Short:** [TBD: Citation+Context]
...
**[105] `path-c-corpus-attested-valency`.**  **Short:** [TBD: Mini-essay]
...
**[106] `path-c-corpus-attested-valency`.**  **Short:** [TBD: Mini-essay]

**Deployments:** Chapter 11 §11.2 ¶ — methodology disclosure for the Path C operational definition; Chapter 11 §11.3 ¶ — anchors the 67.6% tier-share number.

Path C operationalizes a *dhātu*'s reactivity as **corpus-attested combinatorial valency** — the count of distinct (*upasarga*, *pratyaya-class*) pairs that produce attested forms in the reference Sanskrit corpus. The reference corpus is the Digital Corpus of Sanskrit (Hellwig, 2010–2024; CC BY 4.0), accessed through the `OliverHellwig/sanskrit` GitHub mirror at the 2026-05-18 commit; 15,900 CoNLL-U files; 1,007,361 VERB-tagged tokens; 3,839 unique bare *dhātavaḥ* attested across the corpus. *Pratyaya*-class is normalized as a coarse approximation of the Pāṇinian *pratyaya* space — finite forms as the tuple (Tense, Mood, Voice) e.g. *fin:Pres+Ind+Act*; non-finite forms as the VerbForm value e.g. *nfin:Part*. Path A correlation (matched MW subset of 121 *dhātavaḥ*): Spearman ρ = +0.6647. The full reproducibility bundle — scripts, derived data, methodology, sensitivity tests — lives in `analysis/ganah/` with synthesis in `FINDINGS.md`. See dossier for sub-corpus extraction details, the Bhagavadgītā-not-in-DCS substitution note, and the Rāmāyaṇa-as-smriti-epic decision.

**[107] `varga-column-as-engineering-axis`.**  **Short:** [TBD: Mini-essay]
...
**[108] `cross-gana-column-distribution`.**  **Short:** [TBD: Mini-essay]
...
**[109] `inherent-vowel-secondary-axis`.**  **Short:** [TBD: Mini-essay]
...
**[110] `cross-corpus-invariance`.**  **Short:** [TBD: Mini-essay]
...
**[143] `speculation-theory-asuric-certainty`.**  The chapter is not attacking speculation. It is attacking the asuric conversion of speculation into authorized certainty: one civilization's conjecture becomes "theory," while the civilization under study has its categories demoted to "belief."
