...
Yāska's *Nirukta* states the relationship in four words: **नामान्याख्यातजानि (*nāmāny ākhyātajāni*)** — nominal words arise from verbs.[NOTE: nirukta-namany-akhyatajani] The formula describes word formation. The atom expresses an action first, and the nominal form crystallizes from it.
...
| **Lexicon-Dependent Conlang** | A need exceeds the original plan or finite starting inventory. | A creator, founding document, later authority, or speaking community supplies additional material. | External additions enlarge the project. |
| **Generative Architecture** | Speakers derive and compose expressions for the new circumstance through the internal architecture. | The designed atoms, affixes, compounds, and grammatical relations guide generation. | The language can express something new without waiting for another listed word. Its response to entropy depends upon how the architecture is preserved. |
...
Esperanto and Sanskrit both begin inside Generative Architectures. Esperanto underwent **vivification** when communal use began changing the engineered language itself. Sanskrit's two domains produce a different result: the *laukika* domain allows new expression, while the *vaidika* domain preserves the invariant calibrant. **Vivimorphosis** differs from vivification because Sanskrit itself does not move into another quadrant. A particular Sanskrit *śabda* crosses into a natural language, becomes a *bīja*, and takes organic form there while the calibrated Sanskrit molecule remains available in Sanskrit.
