...
The survey shows a bounded empirical field. Retroflexion is near-total. The five-zone axis is near-total. Voicing and nasal contrasts are widespread. Aspiration is strongly present, though not uniformly native in every region. The field is broad, coherent, and geographic.
...
The generative engine produces meaning distinctions through small sound differences. One phoneme changes and the word changes. If two phonemes drift too close acoustically, minimal pairs collapse. A single phoneme merger does not destroy one word; it damages every derivation, compound, and inflection that depends on that distinction.
...
Outside the subcontinent, retroflex consonants are absent, marginal, or limited to special histories. Inside the subcontinent, they are productive, ordinary, and structurally central.[NOTE: retroflex-global-distribution]
...
- Forward to **Epilogue**: the *"Guess which one has lasted?"* polemic hammer on architecture-vs-authority — load-bearing thematic close for the book.
