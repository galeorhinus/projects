...
- **The orthodoxy has not opposed the engineered Sanskrit thesis. It has functioned to ensure that no engineered Sanskrit thesis could be formed.** The defense is structural pre-emption, not argued position. Chapter 2.
...
- **The Indic tradition named the engineered audiograph *akṣara*. The book coins the English equivalent.** *Akṣara* (अक्षर) — *that which does not decay*, from *a-* + √*kṣar* — is the Sanskrit name for the writing-and-utterance primitive (the syllable rendered as glyph); the same word names *Brahman* (*Gītā* 8.3). The orthodoxy classifies the Indic writing system as *abugida* (a name borrowed from Ge'ez) and refuses the engineering-content category *akṣara* names. The book coins ***audiograph*** (Latin *audi-* + Greek *-graphia*, parallel to *photograph*) — the audiograph is the akṣara — and ***audiography*** for the system the *varṇamālā* specifies. Photography is engineered light-capture (1839, named, celebrated); audiography is engineered sound-capture (many thousands of years earlier, never named in the orthodoxy's vocabulary). Pairs with *Auditure* as the book's two sound-anchored engineering coinages. The orthodoxy's six-way script typology refuses the seventh category (audiography), to which one engineered lineage (~1.5B users) plus one independent reinvention (Hangul) belong. Chapter 8 §8.5, Chapter 14 §14.3, and Appendix Part 3.
