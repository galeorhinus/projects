...
Structurally adjacent to the *outward-absorption* move Ch3 §3.4 names (the church of progress absorbing contemporary tradition-internal scholars by elevating them into the priesthood). Heroic erasure is the same mechanism applied to a historical figure rather than a contemporary one — the elevation simultaneously sanctifies the orthodoxy's reading (Pāṇini-as-named-Indian-authority) and erases what the named figure was actually operating within. Deploy where the polemic critiques a founder-myth narrative.
