...
A trade-nonfiction polemic on the engineered architecture of *saṃskṛtam* — *perfectly synthesized* or *wholly created* — and the dismantling of the Western philological framework that has held its dogma against thousands of years of *guru-shishya* lineage-chain evidence. The book argues at two connected scales. Sanskrit's sounds, atoms, grammar, and generative reach demonstrate its internal engineering. The greater engineering feat is the living Vedic system that has kept Sanskrit calibrated against unintended entropy and deliberate asuric attack for thousands of years. This thesis explains what migration-and-decay accounts cannot. The book also argues that the racial Arya thesis, the PIE reconstruction project, and the family-tree taxonomy of Indian languages are all unvalidated inferences the book unbuilds.
...
| `manuscript/as_endnotes.md` | Expanded endnote prose. Stub names key the entries. |
...
| `working/40_reference/sources/as_source_registry.md` | Digital evidence registry. Every digital source used in endnote verification gets a stable ID, exact URL, access date, archive path, and checksum where applicable. Endnotes link to these records through hidden `SOURCE-RECORDS` blocks, which `build_book.py` removes from every reader-facing build. |
...
| `as_reference.yaml` (root), `companion/as_reference_front.md`, `companion/as_reference_*.md` | Source files for the *Source and Reference Companion* — the separate volume carrying the full long-form endnotes. Parallels `as_book.yaml` + the manuscript files for the main book; assembled by `build_book.py`'s reference phase. |
...
**Organizing principle (2026-08-17).** Top-level directories name the *job* they serve, and `working/` holds exactly one job: developing the manuscript. A directory earns top-level placement when it serves a different job, regardless of size. The four jobs: **write the book** (`manuscript/`, `companion/`, `reference/`, `figures/`, `analysis/`, `concepts/`, `working/`), **build the artifacts** (`build_book.py`, `build_html.py`, `templates/`, `filters/`, `build/`), **get readers reading it** (`Caddyfile`, `deploy.sh`, `server/`, `hypothesis/`, `web/`), and **get the book into the world** (`cover/`, `outreach/`).

**One directory per publication.** This repository produces more than one book: the full *Atomic Sanskrit* (`manuscript/`) and the *Source and Reference Companion* (`companion/`), with a concise edition planned (`concise/`, created when drafting starts — see `working/10_active/as_atomic_sanskrit_concise_companion_plan_codex.md`). Each publication owns its prose directory; each has its own metadata yaml at root (`as_book.yaml`, `as_reference.yaml`). Genuinely shared assets stay at top level: `figures/`, and `cover/_shared/`. When a third book becomes real, generalize `build_book.py` to a `--book` flag rather than adding a third hardcoded metadata constant.

**Path-handling rule for the build scripts.** `as_book.yaml`'s `assembly:` entries carry directory-qualified paths (`manuscript/as_1_02_botanical.md`). Any logic that tests a *filename prefix* — `slug_for()`'s `as_part_` test and its prefix-stripping regex, the end-matter grouping's `as_2_` / `as_3_` tests, the `as_endnotes.md` equality check — must take `Path(...).name` first. Skipping that does not raise; it silently produces a different URL slug for every page, changing every live `/as/book/<slug>/` link.
...
| `manuscript/` | **The full book's prose.** Front matter, Part openers, body chapters, epilogue, appendix parts, and `as_endnotes.md`. Reading order is declared in `as_book.yaml`'s `assembly:` block — edit order there, never in the build scripts. |
| `companion/` | **The *Source and Reference Companion*'s own prose** — `as_reference_front.md` plus the reference-only technical appendices. Its long-form endnote bodies come from `manuscript/as_endnotes.md`, the same file the printed book draws `**Short:**` forms from. Metadata in `as_reference.yaml`. |
| `cover/` | Cover matter — the books' own descriptions of themselves, not campaign material. Split per book the way `figures/` splits per chapter: `cover/manuscript/` holds the full book's three jacket-copy variants (`jacket_copy_question_led.md` is canonical, per `build_html.py`'s `JACKET_COPY_SRC`) plus the longer `website_copy_questions_answers.md` sibling; `cover/_shared/` holds what every volume shares (`parag_bio.md`). A second book gets `cover/<its-dir-name>/` when it has copy. Rendered by `build_html.py` as the landing-page copy plus gated reviewer pages at `/as/jacket-copy/<slug>/`; may later be bundled into the PDF by `build_book.py`. |
...
**All manuscript files below live in `manuscript/`** (moved there 2026-08-17 when the repository split one directory per publication); they are named by basename here for readability. Front matter: `as_0_01_preface.md` (Preface), `as_0_01_prologue_eclipse.md` (Prologue — *The Eclipse*, now closing with *The Śaṅkha Sounds*), `as_0_02_acknowledgements.md` (Acknowledgments — author thanks, expandable), and `as_0_04_note_on_notes.md`. (The former Prologue `as_0_03_prologue.md`, *The Prosecution*, is **archived**: its content was re-narrated into the Part I opener when the courtroom frame was retired for the eclipse spine — see `working/40_reference/decisions/eclipse_spine_conversion_plan.md`.) Body chapters: `as_1_00_seekers.md` (Ch 0) through `as_1_20_life_after_pie.md` (Ch 20). Epilogue: `as_2_01_epilogue.md` (renamed 2026-05-19 from *The Atomic Corollary Going Forward* to *Make the World Ārya*). Appendix parts: `as_3_01_baking.md` through `as_3_10_glossary.md`. **Book division structure**: Front Matter / Parts I–VII / Epilogue / Appendices. The book runs on the **eclipse spine** (Sūrya / Svarbhānu / Rāhu / the Atris); the former courtroom frame is retired. Part V now closes with Ch 16 (*One Architecture, Two Domains*). Part VI contains Chs 17–19 (*Dispelling Rāhu*); Part VII contains Ch 20 alone. Each Part carries an italicized eclipse-arc subtitle; full map in `working/40_reference/decisions/eclipse_spine_conversion_plan.md`.
...
The word *demon* never renders *asura* (असुर) in the book's own prose. *Demon* imports the Christian / Abrahamic category of a supernatural evil being onto an Indic word whose Rigvedic uses cross praised and hostile contexts. When a passage presents an *asura* as an opponent, use ***adversary*** or ***antagonist***. When it praises an actor as *asura*, retain the Sanskrit word without imposing a hostile meaning. The book reserves ***asuric*** for the recurring mode of action that conceals, contains, deceives, or consolidates power. The diagnosis follows conduct, not the title alone.
...
*Scripture* (Latin *scriptura*, *a writing*) never designates the Vedas or the Indic corpus in the book's own prose. The word encodes the Abrahamic written-holy-book frame — sacred content fixed in an authorized written text, controlled by an institution — the *foundational dogma*'s engineering choice the book contrasts against (Ch 13, Ch 14). The Vedas are श्रुति (*śruti*, *the heard*), not written scripture; the custody is *chandas* + *śruti* + the *guru-shishya* lineage-chain, not a controlled manuscript. Calling the Vedas *scripture* concedes the category the book exists to refuse.
...
**Governing policy:** `working/40_reference/workflows/sanskrit_devanagari_iast_typography_policy.md`. Read it before formatting any Sanskrit term in the manuscript; what follows is the compressed version.

Devanagari is the primary written form of Sanskrit in this book. IAST lets a reader who cannot read Devanagari pronounce and identify the same word. Use **Devanagari followed by italic IAST in parentheses** when an unfamiliar technical term is introduced, when IAST can mislead pronunciation, when the exact Sanskrit form is evidence, or when an important term returns after a long gap:

> Each धातुः (*dhātuḥ*) carries a stable semantic capacity.

Once a word has been established locally, a familiar or easily read form may stand in IAST or its conventional Indian or English spelling when another full pair would burden the prose. Forms such as शङ्ख (*śaṅkha*), छन्दस् (*chandas*), प्रातिशाख्य (*Prātiśākhya*), and अपभ्रंशः (*apabhraṃśa*) especially benefit from the pair; familiar words such as Sanskrit, Veda, mantra, yoga, karma, *Sanātan, sat, asat, māyā, ārya,* and *sandhi* do not require Devanagari on every occurrence. This is an editorial judgment, not a mechanical character test.
...
**This is a different operation from binding a Sanskrit term to its English gloss.** Pairing Devanagari with IAST (धातुः / *dhātuḥ*) gives pronunciation access. Pairing a Sanskrit term with its English equivalent (*dhātuḥ* — semantic atom, *dhātuḥ* ↔ atom) is the identity-statement governed by Displacement coinage below, which keeps its 3–6-per-chapter rhythm and stands solo between those moments. Do not conflate the two.
...
**Deployment pattern:** first use in a chapter or section names both — for example, "the accusative, द्वितीया विभक्ति (*dvitīyā vibhakti*)." Later mentions may drop the English explanation and, once the term is secure locally, may use the shorter Sanskrit form. Repeat the full Devanagari-IAST pair when the exact grammatical form is evidence or when the reader needs the pronunciation again. Examples: *the accusative, द्वितीया विभक्ति (dvitīyā vibhakti)*; *the optative, लिङ्लकार (liṅ-lakāra)*; *the Vedic subjunctive, लेट्लकार (leṭ-lakāra), deployed only in छन्दस् (chandas)*; *the dual, द्विवचन (dvivacana)*.

### Plain English primary; Sanskrit anchor when useful

No academic Greek/Latin abstract adjectives in the book's own prose — no *liturgical*, *pedagogical*, *exegetical*, *homiletic*, *hermeneutic*, *soteriological*, *eschatological*. Plain English naming the activity: *teaching*, *training*, *worship*, *recitation*, *commentary*, *interpretation*. Pair with a Sanskrit anchor on first use where useful: उपदेश (*upadeśa*), भाष्य (*bhāṣya*), पाठ (*pāṭha*), जप (*japa*), प्रवचन (*pravacana*), शिक्षा (*śikṣā*). Example: *liturgical mode* → *recitational mode* (or *pāṭha-mode*). The general rule extends: *list* not *catalog*, *start* not *commence*, *use* not *utilize*, *show* not *demonstrate*, *make up* not *constitute*, *help* not *facilitate*.
...
The book's own discourse and the standing architecture run in the **simple present** (the gnomic / structural present): *the varṇamālā maps*, *the Aṣṭādhyāyī transforms varṇāḥ*, *Pāṇini's grammar keeps every form derivable*, *the dhātuḥ survives activation*, *the corpus applies these rules*. Treat the Vedas, the *Dhātupāṭha*, the *Aṣṭādhyāyī*, and the corpus as ever-present artifacts whose procedures are readable now.
...
**Abstract *measure* is banned.** Do not use *measure* as a vague substitute for a calibrant, standard, architecture, quantity, boundary, test, or basis of comparison. State the concrete noun instead: the Vedas preserve Sanskrit's architecture; a calibrant gives participants a shared standard; a table records a count; meter fixes a syllable or *mātrā* pattern. Preserve *measure* only for literal measurement, an explicitly defined mathematical or metrical quantity, an exact quotation, a bibliographic title, or a source term whose wording must remain unchanged.

***Gloss / glosses / glossed / glossing* is banned.** State the specific act instead: translate the word, define the term, explain the passage, identify the alternate name, or add explanatory notes. Preserve the word only inside an exact quotation or bibliographic title.

**South Asia / South Asian is banned as the book's geographical terminology.** Use **the Indian subcontinent**, **subcontinental**, or the specific country, region, language, or population. Preserve *South Asia / South Asian* only when it forms part of an exact quotation, bibliographic title, institutional name, or another source's technical label that must be reproduced accurately.
...
**Prefer direct identification when introducing examples, sources, and categories.** Write *The first example is the opening mantra of the Ṛgveda* rather than *The opening mantra supplies the first example*. The second construction makes the source perform an empty abstract action and forces the reader to translate *supplies* back into *is*. A source may *state, explain, describe,* or *show* something when the verb identifies what the source actually does. Avoid making a source *supply, provide, offer, present,* or *introduce* an example or category when direct identification states the relation more plainly.
...
| *"The position the book advances rejects the codification myth."* | *"Pāṇini decoded and documented Sanskrit's architecture, while the pyramid reversed the direction of his work through the codification myth."* |
...
- ***cartel*** — the strongest term (coordinated commercial collusion). **Ration deliberately** — reserve for one or two heavy uses per prosecutorial chapter (Ch3 §3.5, Ch18 §18.4, Appendix §3). Overuse cheapens it.
...
|---|---|---|---|
| ***Engineered*** | Sanskrit and the Vedic calibration system | manifest | The engineering thesis operates at two connected scales. The first is Sanskrit's internal architecture: *varṇamālā*, *dhātupāṭha*, grammar, and generative reach. The greater feat is the system that keeps that language calibrated against two enemies at once. Recitation, meter, two complementary domains, distributed transmission, and correction resist unintended entropy and deliberate asuric attack. ***Engineered* is an empirical-descriptive judgment** about what remains observable today, not a biographical claim that a named human agent-class performed the engineering. The दृष्टाः (*dṛṣṭāḥ*, *seers*) are the lineage-named human conduit who received the Vedas; they are *mantra-dṛṣṭāḥ*, not *mantra-kartṛs*. No separate agent-class is hypothesized between the eternal *śabda* and the *dṛṣṭāḥ*; the मीमांसा (*Mīmāṃsā*) doctrine of अपौरुषेयत्व (*apauruṣeyatva*) — Jaimini's *Mīmāṃsā Sūtra* 1.1.5, Śabara's *Bhāṣya*, Kumārila's *Ślokavārttika* — is the lineage-chain's own anchor for this position. The engineering thesis is the empirical face of *apauruṣeyatva*: what we observe today as engineering is what the eternal *śabda* manifests as. |
| ***Encoded*** | The Vedas | embedding | carry Sanskrit's sounds and grammar in a **transmissible and immutable** form while anchoring the larger calibration system. छन्दस् (*Chandas*), श्रुति (*śruti*), recitation procedures, and the गुरुशिष्यपरम्परा (*guru-shishya-paramparā*) keep the architecture available across generations. The encoding is visible, not concealed — anyone fluent in Sanskrit can read it. |
| ***Decoded*** | Yaska, Sthaulāṣṭhīvi, Śakapūṇi, Śākalya, the pre-Pāṇinian grammarian roster, Pāṇini, the *Prātiśākhya* / *Śikṣā* / *Mīmāṃsā* disciplines | extraction | recover the explicit specification from the encoded corpus. व्याकरणम् (*Vyākaraṇam*, *unfolding apart*) is Sanskrit's own disciplinary word for what the *vaiyākaraṇāḥ* did. |
...
***Asuratva* (असुरत्व) — the asuric operating mode.** A recurring mode of action through which an actor or institution consolidates power, deceives to preserve authority, or obstructs light and circulation. Chapter 3 §§3.6–3.7 establishes this category through *māyā*, *asura*, and *viveka*.

Sanskrit has two distinct formations on the sound-form ***asura***: ***asu-ra***, the breath-bearer, and ***a-sura***, the book's diagnostic name for an actor who turns power against radiance. We do not know whether the Ṛgveda uses both words because the surrounding words do not always tell us which one it is using. State that thesis upfront. Then set the lexical uncertainty aside, establish action versus faction through the actors' purposes and actions, and return later to the two passages that demonstrate why the question remains unresolved.

The absence of an independently recorded **सुर (*sura*)** in the Ṛgveda does not eliminate ***a-sura*** as a possible formation. Sanskrit is generative: the Ṛgveda uses **अदब्ध (*a-dabdha*)** forty-eight times even though independent **दब्ध (*dabdha*)** does not appear there. Use that comparison to reject the pyramid's word-list objection. Keep the conclusion exact: generativity establishes that a Rigvedic occurrence ***could*** be ***a-sura***; it does not establish which occurrence is. This is a distinction between **possibility** and **assignment**.

Chapter 3 §3.6 presents this examination as an exercise of ***viveka***. *Māyā* is power, and its alignment follows its use. ***Āsurī māyā*** conceals the relevant action and projects a factional identity in its place. Section 3.7 defines the wider containment pattern through Vṛtra, Vala and the Paṇis, Svarbhānu, distributed power, and the RV 3.55 refrain. *Asuratva* has a characteristic geometry — the pyramid — and a characteristic substrate — *tamas*, the *guṇa* of inertia, darkness, and obscurity. Chapter 4 applies this diagnosis to Western philological dogma and the *church of progress*. Reference Ch 3 §§3.6–3.7 and Ch 4 instead of redeveloping the full diagnostic.
...
Argue, don't survey. Commit to positions. Combative on substance, civil on persons. Dichotomy → reframe is the signature move — set up the Western binary, show the binary is the wrong frame, offer the orthogonal third frame rooted in Indic thought. Layered, connective sentences build the case by carrying the logic in the syntax, and a section ends when its last load-bearing thought is complete. Open chapters with wordplay (running it as structural spine across the chapter) or scene (then pivot). Engineering vocabulary is natural — *orthogonal*, *integration vs. discreteness*, *dispersive*, *rotational symmetry*, *triad*, *two-pronged attack*. Trust the metaphor when it lands; send the technical proof to the chapter whose territory it belongs to or to an endnote. Familiar examples from the reader's daily speech anchor the abstract argument. Do not use *operate, operates, operated, operating,* or *operation* as a generic substitute for a concrete action; name what joins, derives, generates, preserves, distinguishes, restricts, checks, transmits, or applies. Preserve specifically identified grammatical or mechanical operations and established terms such as *operating system*. Do not make ideas, distinctions, categories, methods, arguments, frameworks, conclusions, sections, or architectures *arrive*; state what creates, introduces, establishes, precedes, produces, reveals, or reaches them. Preserve literal arrival and migration. Do not say that a figure, example, count, method, or section *makes* an abstract pattern, procedure, category, calculation, contrast, or architecture *visible*; state what it displays, compares, counts, distinguishes, establishes, or reveals. Preserve literal sight, visible writing, audiographs, and the eclipse's visible-versus-concealed contrast.
