Now let me read the TOC notes file — the central document for the audit.Let me briefly check the Ch16 and Epilogue notes files to confirm Wave 1/2 vocabulary status and Epilogue current shape.I've read through the handoff (Session 3 continued at top), the todo list (section A immediate task), the voice calibration in the system prompt, the sidebars file with its voice-calibration alert at the head, and the TOC notes against all six sidebars. I also spot-checked `as_ch15_notes.md`, `as_2_01_epilogue_notes.md`, `as_1_02_strategic_notes.md`, and the Ch2 draft to verify current state. Audit follows.
...
**Recommendations:**
- Create `as_1_07_adivadya_notes.md` per `as_todo.md` section C item 1, seeded with Sidebar 1's *ārya* / *mleccha* phonetic-pedagogical framework as the chapter's organizing argument and Sidebar 4's Savarkar / Samarth Ramdas anecdote as the chapter opener.
...
The Ch2 v1 draft (`as_1_02_strategic.md`) has §2.4 as a seven-paragraph progress-pillar section with the hammer "*This is the pillar that holds*." The notes file (`as_1_02_strategic_notes.md`) treats §2.4 as "The Progress Crisis (the contemporary pillar — the most potent threat the engineered Sanskrit thesis poses today)." The kālacakra-vs-linear-progress contrast is established. The "progressivism" framing is present without the *illiberal* / *arāvan* terminology.
...
**Recommendations:**
- Update `as_1_02_strategic_notes.md` per `as_todo.md` section C item 2 to make §2.4 the operative-terminology section for *illiberal* / *arāvan*. Sidebar 5 supplies the etymological argument and the surface-generous / structurally-ungenerous distinction.
...
**Create:**
- `as_1_07_adivadya_notes.md` — seed with Sidebar 1's *ārya* / *mleccha* phonetic-pedagogical framework as the chapter's organizing argument, Sidebar 4's Savarkar / Samarth Ramdas anecdote as opener, the triple-convergence framing, and the Hollywood-caricature material as supporting evidence.
...
**Update:**
- `as_1_02_strategic_notes.md` — make §2.4 the load-bearing terminology-introduction section for *illiberal* / *arāvan* per Sidebar 5's two-stage structure (Rigvedic mantra withheld; the Epilogue lands it). Add Sidebar 4's slap-back to §2.2.
- `as_2_01_epilogue_notes.md` — substantial rework. Add Wave 3 framework, full mantra (*kṛṇvanto viśvam āryam | apaghnanto arāvṇaḥ*), loop-close to Ch2 §2.4, reader-as-Wave-3-*ṛṣi*-in-potentia framing, Wilson / Griffith mistranslation case study. The mantra-landing replaces the benefits-catalog as the structural close; the *Second Shānti* signal stays as forward-pointer.
...
- **Epilogue (Wave 3 mantra-landing)** — completes the two-stage Ch2 / Epilogue arc. Highest structural payoff. Requires `as_2_01_epilogue_notes.md` substantial rework first. The notes-rework plus prose-draft is the heaviest two-pass session of the three options. Lands the book's structural close — high stakes for getting the register right.
- **Ch7 (retroflex / *mūrdhanya*)** — requires `as_1_07_adivadya_notes.md` creation first; that's a from-scratch notes file. Sidebars 1 and 4 supply the structural spine but the notes file does not yet exist. Two-pass session. Lower priority than Ch4 / Ch16 / Epilogue per `as_todo.md` section A item 2.
...
- Ch7 v1 (still queued; needs `as_1_07_adivadya_notes.md` first; needs Buddha-Assalāyana citation as supporting authority)
...
- `as_1_03_fourth_abrahamic_notes.md` — created as stub with six-term cluster, four-paragraph introduction, deployment plan
...
- `as_2_01_epilogue_notes.md` (already substantially updated this session with Section 8; main 2.x outline integration deferred)
- `as_1_02_strategic_notes.md` (entry in `as_todo.md` flags update needed; deferred)
- `as_1_07_adivadya_notes.md` (still doesn't exist; deferred)
...
Each deployment integrated into existing prose, not term-substitution. Deployment plan from `as_1_03_fourth_abrahamic_notes.md` Section 5 followed.
...
- `as_2_01_epilogue_notes.md` (three cluster deployments in Section 8 + chapter cross-references updated)
- `as_toc_notes.md` (chapter headings 3-15 → 4-16; renumbering note replaced with completion note)
- `as_1_03_fourth_abrahamic_notes.md` (deployment-plan table updated; §16.X → §17.X; "(current Ch16)" parentheticals dropped)
...
- `as_1_03_fourth_abrahamic_notes.md` — deployment-plan table updated, but the file itself can be renamed `as_ch03_notes.md` on a future pass to match the new chapter file.
...
- `as_2_01_epilogue_notes.md`: chapter cross-reference list updated to name both Ch17 + Ch18. Section 8 references to §17.5 / §17.6 updated to §18.3 / §18.4.
...
- `as_1_03_fourth_abrahamic_notes.md`: deployment plan table updated — Diasporic Wave deployment now at Ch18 §18.3.
