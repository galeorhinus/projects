...
> **Status:** The Ch7 split was executed in Session 9 (Tuesday, May 12, 2026). Original Ch7 is now two distinct chapters: **Ch7 — *The World's First Instrument*** (`as_1_07_adivadya.md`, descriptive register, 4,557 words) and **Ch8 — *Mapping the Mouth*** (`as_1_08_mapping_mouth.md`, full polemic register, 5,100 words). The original Ch8 (*Flexing the Retroflex*) has been renumbered to Ch9 (`as_1_09_retroflex.md`). All downstream chapters renumbered +1: original Ch15/16/17 → Ch16/17/18.
>
...
### Chapter titles
- **Ch7**: *The World's First Instrument*
- **Ch8**: *Mapping the Mouth*
...
### File strategy
**Option C** — Single file `as_1_07_adivadya.md` containing both chapters as distinct chapter blocks (`# Chapter 7 — The World's First Instrument` and `# Chapter 8 — Mapping the Mouth`), separated by clear delimiter. When ready to split: extract by chapter heading into two files. Mechanical operation.
...
- **Ch8** reveals the **inventory** — the specific 25 + 14 + 4 + 4 sounds and letters the *varṇamālā* committed to.

This rule is what makes the split pedagogically airtight. The reader finishes Ch7 knowing what kinds of sounds humans make and what Sanskrit calls each kind; they do not yet know which specific sounds Sanskrit selected. Ch8's reveal lands fresh.

**Concretely, this means:**
- Ch7 uses *sparśa*, *swara*, *antaḥstha*, *ūṣman* as category-words (the kinds of sounds)
- Ch7 does NOT use *varṇa*, *varṇamālā*, *varga*, or any specific Devanagari letter (क, ख, ग, अ, इ, etc.)
- Ch8 introduces *varṇa* and *varṇamālā*, names the inventory counts, shows specific letters

### The Ch8 crystalline thesis
> **The names of the sound happen to be the sound itself.**

Placement: §8.2 close (after the inventory is named and the *architects chose a selection* observation lands). The chapter's deepest engineering claim about the *varṇamālā* — that the script does not represent sound, it constitutes the sound's specification.
...
**Pivot between parts:** §7.6 turns the diagram over — same anatomy, Sanskrit naming system. No transitional fanfare. The chapter simply continues describing the apparatus, switching vocabulary.
...
- Diagrams may include Sanskrit anatomy labels (FIGURE 7.2)
...
- **Ch7**: Anatomical-region names are primary. The vocal tract's ~17 cm length is established as a fact. cm scale appears in FIGURE 7.1 (as a scale at the bottom of the diagram) but not as primary labels in prose. The reader notices that the vocal tract has measurable length without being asked to memorize specific cm values for specific consonants.
- **Ch8**: cm-axis framing is where the snap-to-grid argument lives (FIGURE 8.1). Sanskrit's five *sthāna* plotted at approximate cm positions; English consonants at their approximate cm positions; Arabic pharyngeals at the back-of-range. cm values are approximate; the relative spacing is the argument.
...
**Approximate cm positions for English consonants** (for FIGURE 8.1 design reference):
...
**Proposed FIGURE 7.3** — a visualization showing where 3–4 language groups concentrate their consonant inventories along the vocal tract. Goal: make *every language is a selection* visible at §7.5 rather than just asserted. Patterns emerge from the visual comparison.

**Design decisions (locked):**
- **Language groups exclude Indic.** Sanskrit/Hindi inventory is Ch8's reveal — must not appear in Ch7.
- **cm-axis** (not qualitative IPA front-to-back). Consistent with FIGURE 8.1's cm framing and the acoustic-phonetics modeling tradition.
...
- Visualization lands at §7.5 (*The Instrument's Range*) — the section where the *every language is a selection* observation is made
- May benefit §7.3 (*How Consonants Are Made*) and §7.4 (*How Vowels Are Made*) by foreshadowing the comparison

**To-Do marker for chapter prose:**
- §7.5 should include `[TODO: FIGURE 7.3 — language-hotzones visualization. Design pending. See notes file for decisions and open questions.]`

### The Pre-Pāṇinian Classification Framework

The pre-split notes mentioned *Prātiśākhya* and *Śikṣā* as the engineering tradition but did not document the **actual classification system** those texts use. This section corrects that gap. Some of this framework belongs in Ch7 Part 2 (descriptive); the full framework is what Ch8's "Pāṇini was second" polemic refers to.
...
**Frits Staal's classical observation** — relevant for Ch8's polemic:
...
- Part 1 (the structural comparison) justifies presenting the 5×5 grid through multiple complementary visualizations in §8.7, including a periodic-table-styled rendering (FIGURE 8.3). The structural parallel between the *varga* grid and the periodic table is genuine — both systems decompose their units into constituent parameters and place each unit at a unique coordinate.
...
This is the chapter's signature move regarding Staal — agree on the structure-of-the-system, disagree on the history-of-the-system. The polemic lands at §8.5 (Engineering Precedes Pāṇini); the visualization payoff lands at §8.7 (Reading the Varṇamālā).

**The 5×5 grid visualization plan** (developed in §8.7):

The grid is presented through multiple complementary views in Ch8 itself — control panel, periodic-table style, matrix table, and possibly vocal-tract overlay. Each view makes a different aspect of the engineering visible; the reader sees the same data through several visual idioms; the structure becomes inescapable. Full design in §8.7 of these notes.

**No conflict with Ch9 *dhātus* reservation:** Ch9 uses the periodic-table-of-elements metaphor for *dhātus* (verbal roots — units of meaning). Ch8 uses the periodic-table-style visualization for the *varga* grid (units of sound, anatomically decomposed). Two distinct deployments at different linguistic scales — sound and meaning. The metaphors complement rather than conflict. Ch9 takes one row of the Ch8 grid (the retroflex *varga*) and develops it; Ch9 does not re-present the full grid.
...
**Vocabulary distribution between Ch7 and Ch8:**

| Term | Ch7 introduces? | Ch8 uses? | Notes |
|---|---|---|---|
| *Sthāna* | Yes (§7.7) | Yes — recap and deploy | Five anatomical contact-stations |
| *Karaṇa* | Yes (§7.7, light) | Yes — recap | Active articulator (tongue, lips) |
| *Prayatna* | Yes (§7.9) | Yes — deploy as engineering axis | Effort / manner |
| *Ābhyantara prayatna* | Yes (§7.9, brief) | Yes — light recap | Type of constriction |
| *Bāhya prayatna* | Yes (§7.9, brief) | Yes — connects to operating modes | Voicing / aspiration / glottal state |
| *Anupradāna* | Mention only (§7.7) | Yes — deeper treatment | Phonation |
| *Śvāsa / Nāda* | Mention only | Yes — connects to *aghoṣa* / *ghoṣa* | Breath / resonance |
| *Vivṛta / Saṃvṛta* | Optional in Ch7 | Yes — glottal state framing | Open / closed glottis |
| *Spṛṣṭa* | Yes (§7.8) | Yes — recap and use | Full contact (= *sparśa*) |
| *Aspṛṣṭa* | Yes (§7.8) | Yes — recap | Non-contact (= *swara*) |
| *Īṣat-spṛṣṭa* | Mention in §7.8 | Yes — *antaḥstha* explanation | Light contact (= *antaḥstha*) |
| *Īṣat-saṃvṛta* | Mention in §7.8 | Yes — *ūṣman* explanation | Light closure (= *ūṣman*) |

This is what Ch7 Part 2 will document descriptively. Ch8's Pāṇini-was-second polemic in §8.5 deploys this full framework as the engineering Pāṇini inherited; §8.7 presents the 5×5 grid through multiple complementary visualizations.

### Cross-language examples (Part 1)

**Languages to thread through §7.3 and §7.4** (4–6 total, not exhaustive):
...
**Languages NOT to use in Ch7:**
- Sanskrit (the inventory is Ch8's reveal)
- Hindi (same — would reveal *varṇamālā*-adjacent inventory)
- Tamil (will appear in Ch8 re: *alpaprāṇa*-only subset)
...
- **FIGURE 7.1** placeholder — cross-section of human head, English anatomical labels, ~17 cm midline distance shown
- Target: ~450 words
...
- **FIGURE 7.3** placeholder — language-hotzones visualization (3–4 language groups; cm-axis; vowels included; design pending — see Music-tradition framing → To-Do section above for decisions and open questions)
- In-prose to-do marker: `[TODO: FIGURE 7.3 — language-hotzones visualization]`
...
- **FIGURE 7.2** placeholder — same vocal-tract cross-section as FIGURE 7.1, Sanskrit anatomical labels overlaid
- Target: ~200 words
...
- Brief mention: the *Prātiśākhya* tradition classifies sounds along multiple dimensions — *sthāna* (place), *karaṇa* (active articulator), *prayatna* (effort, treated in §7.9), *anupradāna* (phonation: *śvāsa* breath / *nāda* resonance). Sanskrit's classification framework is multi-axis; this reflects centuries of attention to the apparatus
...
- **Ch7 close**: A soft transition. Something like: *The instrument has been mapped, in both English and in Sanskrit. The next chapter takes up the specific selection that one tradition committed to — and the script that encodes it.*
- Target: ~400 words (increased from 300 due to *prayatna* subdivision)

### Figures

- **FIGURE 7.1**: Vocal tract cross-section, English labels. Lips, teeth, alveolar ridge, hard palate, soft palate, uvula, pharynx, larynx, vocal cords, tongue (with apex/blade/dorsum/root visible), lungs, nasal cavity, oral cavity. ~17 cm midline distance from lips to glottis indicated; cm scale at bottom as supplementary reference. The chapter's reference diagram for Part 1.

- **FIGURE 7.2**: Same vocal-tract cross-section as FIGURE 7.1 — same anatomical drawing — with Sanskrit labels overlaid. The five *sthāna* positions (*oṣṭhya*, *dantya*, *mūrdhanya*, *tālavya*, *kaṇṭhya*) marked at their cm-distances from lips. Other anatomical components labeled in Sanskrit (lungs as *prāṇa*-source, vocal cords as *ghoṣa*-source, soft palate as *anunāsika*-controller). The visual demonstration that two naming systems describe the same instrument.

- **FIGURE 7.3** (design pending): Language-hotzones visualization. X-axis = cm distance along the vocal tract (lips at 0, glottis at ~17). 3–4 language groups represented (final selection TBD; Indic excluded — saved for Ch8). Vowels and consonants both shown. Granularity decision (dots vs density) TBD during drafting. Patterns expected to emerge: English heavy in alveolar cluster; Arabic extends back through pharyngeal; Mandarin shifts toward retroflex/palatal; Hawaiian sparse minimal inventory. Anchors §7.5's *every language is a selection* observation visually. **See "To-Do During Drafting" section above for full design state and open questions.**

### Endnote stubs (new for Ch7)
...
- `vocal-tract-cm-modeling` — Fant (1960) *Acoustic Theory of Speech Production*; Story (2005); Stevens & Blumstein (1978); the cm-based vocal-tract modeling tradition in acoustic phonetics research. Justifies the cm-axis representations in FIGURE 7.1, FIGURE 7.3, and FIGURE 8.1
...
### Endnote stubs (existing, partially stay in Ch7)

- `place-of-articulation-sanskrit-terms` — Stays partially in Ch7 (the *sthāna* terms documented in *Prātiśākhya* / *Śikṣā* tradition); the Pāṇini-was-second elaboration moves to Ch8
- `formants-source-filter-theory` — Possibly compresses or moves; depends on how deep §7.4 goes into vowel acoustics. Lean: compress or move to Ch8.
...
**Reused content** (largely from current `as_1_07_adivadya.md` with light editing for new section boundaries):
- §8.1 phonics opener (current §7.1)
- §8.3 snap-to-grid (current §7.4)
- §8.5 Pāṇini-was-second (current §7.3 mid-section)
- §8.6 acoustic engineering depth (current §7.5 engineering payoff)
- §8.7 pipe-organ + *घ* neuro-motor (current §7.5 pipe-organ paragraph)
- §8.8 *mahāprāṇa* and Hindi flap (current §7.5 late)
- §8.9 two-instrument framing + swara temporal cuts (current §7.6)
- §8 close synthesis + hammer (current chapter close)

**New writing required** (light):
- §8.2 *the selection* — connects Ch7's *every language is a selection* observation to the specific *varṇamālā* inventory; introduces *varṇa*, *varṇamālā*, *varga*, the 25+14+4+4 counts; lands the crystalline thesis
- Section-boundary transitions throughout

**Vocabulary Ch8 may assume** (already established in Ch7):
...
**Vocabulary Ch8 introduces fresh:**
- *Varṇa* वर्ण (letter-sound; the load-bearing Ch8 unit)
...
**§8.1 — Mapping the Mouth** (~450 words)
...
- Transition: this chapter asks why Indic scripts work this way — what one tradition built on top of the apparatus Ch7 described

**§8.2 — The Selection** (~600 words, MOSTLY NEW)
- Connect to Ch7: the human vocal apparatus has many degrees of freedom; every language is a selection from that range
- The architects of Sanskrit — operating in the *Prātiśākhya* and *Śikṣā* traditions over many generations — chose specific target locations on the map of the mouth Ch7 laid out
...
- Brief mention of the five *vargas* by their *sthāna* organization (without yet enumerating specific letters in each *varga* — that comes in §8.3 with the *kavarga* example)
...
**§8.3 — Snap to the Grid** (~700 words)
...
- The *kavarga* used as worked example here (since §8.2 introduced *varga*): क ख ग घ ङ as the five operating modes at the *kaṇṭhya sthāna*
...
- **FIGURE 8.1** placeholder (currently FIGURE 7.2): linear vocal-tract diagram with English/Arabic exclusions plotted against the five *sthāna* positions

**§8.4 — The Naming of the Sounds** (~350 words)
- The Sanskrit terminology that names the grid and its sounds
- The *sthāna* names already introduced in Ch7 (*oṣṭhya*, *dantya*, etc.) — brief recap
...
**§8.5 — The Engineering Precedes Pāṇini** (~800 words)
...
- **Deployment of Staal's observation** — endorse-comparison / reject-history mode:
  - Staal's classical observation, drawn from secondary sources (primary source to verify):
    > *Like Mendelejev's Periodic system of elements, the* varga *system was the result of centuries of analysis. In the course of that development, the basic concepts of phonology were discovered and defined.*
  - **Endorse the structural comparison.** The *varga* grid and the periodic table share a logic — units placed at unique coordinates of their constituent components. The structural parallel is genuine and justifies the periodic-table-styled visualizations of the grid in §8.7.
...
- **Forward-pointer**: The 5×5 *varga* grid as visualization — through multiple complementary views including the periodic-table-styled rendering — is taken up in §8.7 (Reading the Varṇamālā), where the grid's operational structure is shown
...
**§8.6 — The Acoustic Engineering** (~650 words)
...
- The four anatomies framework — now deployed as engineering payoff (not just description, which is Ch7's mode)
...
**§8.7 — Reading the Varṇamālā** (~700 words, increased from 500)
...
- **The 5×5 grid through multiple complementary views:**
  ...
  - **View 1 (FIGURE 8.2): Control panel.** *Sthāna* on Y-axis, *prayatna* on X-axis, each cell carries the Devanagari character. The operational reading — the grid as instrument board.
  - **View 2 (FIGURE 8.3): Periodic-table style.** Each consonant in its own bordered cell, arranged in the 5×5 grid styled visually like Mendeleev's table. The anatomical parameters that produce each sound shown as encoded labels — possibly via color, position within the cell, or superscript/subscript notation indicating *sthāna* + *prayatna* combination. The systematic-decomposition reading — the grid as structured decomposition of every *sparśa* sound into its anatomical parameters. This visualization endorses Staal's structural comparison while saying nothing about how the grid came to be — it just shows what the grid *is*.
  - **View 3 (FIGURE 8.4): Matrix table.** Plain rows × columns with cells, the most data-dense rendering. Suitable for reference, possibly placed in an appendix or as a sidebar callout. Provides a clean look-up for any reader who wants to verify what the chapter is describing.
  - **View 4 (possible — FIGURE 8.5): Vocal-tract overlay.** The 25 *sparśa* consonants positioned on a stylized vocal-tract cross-section at their actual *sthāna* locations, with the five *prayatna* operating modes shown as variants at each location. Connects FIGURE 8.1's snap-to-grid framing to the grid's full content.
...
- **Forward-pointer for the retroflex *varga* specifically**: the third row of the grid (ट ठ ड ढ ण) is isolated and developed in Ch9 — the test of *āryatva* and the codification perimeter (what Pāṇini's *bhāṣāyām* bounded out, e.g., the ळ that *Ṛgveda* uses). This is one row of the grid Ch8 has presented; Ch9 takes that row and develops it.

**§8.8 — The Subcontinental Substrate** (~400 words)
...
**§8.9 — Two Instruments** (~500 words)
- Two-instrument framing as chapter-level synthesis: *sparśa* vs *swara* as two modes of the same apparatus
- The apparatus Ch7 mapped as instrument can be played with contact (struck mode) or without (continuous mode)
...
**§8 close — Roman Inventory, Varṇamālā Anatomy** (~250 words)
...
- **FIGURE 8.1**: Linear stretched-out vocal-tract representation from 0 cm (lips) to ~17 cm (glottis). Sanskrit's five grid positions marked. English consonants plotted (interdental *th*, alveolar t/d/n/s/z/l/r, post-alveolar sh/ch/zh). Arabic ع/ح at ~15 cm. Arrows showing snap mechanism. Shows the superset vs the chosen subset.

- **FIGURE 8.2** (View 1 — Control Panel): *Varṇamālā* as control panel. Y-axis = *sthāna* (top-to-bottom: *kaṇṭhya* → *tālavya* → *mūrdhanya* → *dantya* → *oṣṭhya*). X-axis = *prayatna* (left-to-right: *aghoṣa-alpaprāṇa* → *aghoṣa-mahāprāṇa* → *ghoṣa-alpaprāṇa* → *ghoṣa-mahāprāṇa* → *anunāsika*). Each cell with Devanagari + IAST. Plus middle (vowel matrix) and bottom (liminal set) panels. Reads as operational diagram. **Lands in §8.7.**
...
- **FIGURE 8.4** (View 3 — Matrix Table): Plain rows × columns table — most data-dense rendering. Each row a *sthāna*; each column a *prayatna* operating mode; each cell carries the Devanagari character, IAST transliteration, and the four-anatomy decomposition (*sthāna* + *karaṇa* + *ābhyantara prayatna* + *bāhya prayatna*). Suitable for reference. May land in main text body or as an appendix/sidebar — to be decided during drafting. **Lands in §8.7 or appendix.**

- **FIGURE 8.5** (View 4 — Vocal-Tract Overlay, possible): The 25 *sparśa* consonants positioned on a stylized vocal-tract cross-section at their actual *sthāna* locations. The five *prayatna* operating modes shown as variants stacked at each location (or shown in a small inset table at each location). Connects FIGURE 8.1's snap-to-grid framing to the grid's full content. **Lands in §8.7. Optional — include only if it adds genuine clarity, not for completeness.**

### Endnote stubs (carry over from current draft)

- `varnamala-grid-geometry` — moves to Ch8 §8.2 or §8.3
- `place-of-articulation-sanskrit-terms` — partially moves; the Pāṇini-was-second sub-content moves to Ch8 §8.5
- `jones-1786-third-anniversary-discourse` — moves to Ch8 §8.5
- `ipa-1886-founding-1888-chart` — moves to Ch8 §8.5
- `history-of-linguistics-sanskrit-influence` — moves to Ch8 §8.5
- `sandhi-anusvara-assimilation` — moves to Ch8 §8.3
- `vedic-svara-system` — moves to Ch8 §8.9
- `hrasva-dirgha-pluta-matra` — moves to Ch8 §8.9

### Endnote stubs (new for Ch8)
...
| Two-axis description of apparatus | *Sthāna* / *prayatna* canonical pair | Already-known; used to label FIGURE 8.2 axes |
...
- **FIGURE 7.1** and **FIGURE 7.2** are the same anatomical cross-section with different label-overlays. Visual demonstration that two naming systems describe one apparatus.
- **FIGURE 7.3** (design pending) — language-hotzones visualization at §7.5
- **FIGURE 8.1** is the linear-stretched version of the vocal tract — derived from but visually distinct from FIGURE 7.1. Shows the superset/subset relationship that's load-bearing for snap-to-grid.
- **FIGURES §8.2 through §8.5** are the 5×5 *varga* grid presented through multiple complementary visualizations — control panel (§8.2), periodic-table style (§8.3), matrix table (§8.4), and optional vocal-tract overlay (§8.5). The reader sees the same grid through several visual idioms; the structure becomes inescapable.
- **Cross-chapter: the 5×5 grid lives in Ch8.** The full grid (all 25 *sparśa* consonants, all 5 *sthāna* rows × 5 *prayatna* operating modes) is presented in Ch8 through the multiple-view treatment. Ch9 takes one row of that grid — the retroflex *varga* (ट ठ ड ढ ण) — and isolates it as the test of *āryatva*. Ch9 does not re-present the full grid; it takes the row Ch8 has shown and develops it.

### Transition mechanics

- **Ch7 close**: Soft transition. *The instrument has been mapped, in both English and in Sanskrit. The next chapter takes up the specific selection that one tradition committed to — and the script that encodes it.*
- **Ch8 open**: Phonics opener (current §7.1). The transition is not narratively bridged; Ch8 starts cold with a polemic move. The reader who absorbed Ch7 has the vocabulary; Ch8 turns the lens.

This is by design — Ch7 is descriptive, Ch8 is argumentative, and a soft narrative bridge between them would muddy the voice register shift.
...
### Ch7 specific (descriptive register)
...
- **No "engineered" / "engineering" language.** Save for Ch8. In Ch7, use *the apparatus*, *the instrument*, *the anatomy*, *the structure*.
- **No Pāṇini reference.** Pāṇini doesn't appear in Ch7; his work belongs in Ch8's polemic.
...
- **The four-anatomies framework is descriptive in Ch7, polemic in Ch8.** Same content, different register.
...
### Ch8 specific (full polemic register)
...
- The crystalline thesis (*The names of the sound happen to be the sound itself*) is load-bearing — placement at §8.2 close.
...
- For category-words in Ch7 (*sparśa*, *swara*, *antaḥstha*, *ūṣman*): introduce as kinds-of-sounds, with English gloss
- For specific letters in Ch8 (क, ख, etc.): introduce in §8.3 worked example onward
...
2. **Step 2 (done)**: Build new restructured `as_1_07_adivadya_notes.md` (this file)
...
6. **Step 6**: Revise Ch7 per feedback
7. **Step 7**: Assemble Ch8 from existing content into new structure (~3,000–3,500 words)
...
11. **Step 11**: Final integration — single `as_1_07_adivadya.md` with both chapter blocks
