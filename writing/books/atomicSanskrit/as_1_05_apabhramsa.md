...
**Appendix Part 6 — *The Vedic Carrier*** develops each of the eight orthodox drift-claims with its specific example and tabulates the engineering-mode response, after walking three load-bearing Ṛgvedic verses phrase-by-phrase to demonstrate the implicit grammar in operation in the corpus form. The polemic-critical observation landing at the appendix's center: *chandas* (छन्दस्) means *meter*; the *chandasi* mode is *the metrical mode*; everything that distinguishes the Vedic mode from the Classical mode is, fundamentally, what the meter requires.
...
**Endnote stubs in this chapter:** `paspashahnika-apabhramsa-passage`, `vedic-variation-eight-claims`, `rosa-law-2013`, `pinker-euphemism-treadmill`.

**Cross-references:**
- Backward to **Ch 4 §4.5**: the *siddha* axiom is what makes the *apabhraṃśa* defense intelligible (delivered by Ch 4's chapter close).
- Backward to **Ch 1 §1.2 / §1.3**: *hlāfweard → Lord* as the natural-drift example (deployed in §5.5 to contrast with Sanskrit's anti-entropy response).
- Forward to **Ch 14**: the full calibration matrix the §5.5 inventory only sketches.
- Forward to **Ch 18**: the *pratibimba* account of downstream reflection (cited at §5.6); the calibrant-contact argument (named at §5.7 close).
- Forward to **Appendix Part 6 — *The Vedic Carrier***: the detailed per-claim engineering walk for the eight orthodox drift-claims; the three *Ṛgvedic*-verse phrase-by-phrase development.
