...
| `working/eclipse_restructure_plan_final.md` · Ch 4 epigraph queue | Muṇḍaka Upaniṣad 1.2.8 candidate: "blind led by the blind." | `REJECTED AS EPIGRAPH / POSSIBLE §4.5 SUPPORT` — the verse's useful target is self-congratulating learnedness inside *avidyā*, but as an opening epigraph it can sound like contempt toward ordinary people lower in the pyramid. Use only later, if at all, with explicit targeting of self-certifying guides. |
...
| `as_1_05_apabhramsa.md`, `working/as_figure_production_queue.md` · Ch 6 Pass 3 freeze audit | Figure queue still described Figure 6.2 as "three-frame"; Ch 6 §6.8 had drifted to "freezing fluid *varṇa*" in the caste zoom-out. | `RETUNED / FREEZE CANDIDATE` — figure queue now matches the active "three-category" language, and the caste zoom-out is restored to "freezing fluid *jāti*" to preserve the distinction between functional *varṇa* and enumerated social administration. |
