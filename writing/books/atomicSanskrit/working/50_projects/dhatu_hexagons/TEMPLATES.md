...
3. **Scales cleanly.** New templates discovered in expanded corpora (Gītā / Vedic subsets, see `working/10_active/as_author_tasks.md`) can be named on the same pattern: identify the canonical *dhātu*, apply sandhi, suffix *-ādi*.
...
- **Gītā / Vedic subset analysis.** Run the same template-classifier against the ~500 *dhātus* listed for the *Bhagavad Gītā* and the *Vedas* (queued in `working/10_active/as_author_tasks.md`). Compare distributions: does usage match the inventory, or does the inventory exceed actual usage?
