...
- `[x]` **Pass 4 — Polemic Calibration**
  ...
  - Scope: Prologue, Ch1-Ch3, Ch18-Ch19, Appendix Parts 1-3, Epilogue.
...
- `[x]` **Pass 6 — Epilogue Loop Closure**
  ...
  - Scope: Prologue + Epilogue, with spot checks against Ch1, Ch3, Ch19, Ch20.
...
| `[x]` | Ch19 -> Ch20 -> Epilogue | The escalation works: Ch19 convicts, Ch20 supplies the replacement model, Epilogue restores the dharmic frame. | The remedy chapter should not feel skipped in the Epilogue’s opening summary. | Added the bridge: Ch19 closed the prosecution, Ch20 began the remedy, the Epilogue returns the whole movement to dharmic ground. |
...
| `[x]` | Former Ch12 §12.9 | Vivimorphosis expanded beyond assembly into contact-language theory and Chapter 19 territory. | The concept pulled the chapter from its construction spine into the PIE prosecution before the reader reached Part VI. | Resolved 2026-08-22: moved the full mechanism to Chapter 19 §19.7; Chapter 12 §12.7 retains only a short forward pointer. |
| `[x]` | Ch14 §14.1 and §14.3 figures | Ch14 still contains figure placeholders for the four preservation modes and six-layer calibration matrix, while Ch10-Ch12 now use rendered figures. | The architecture spine becomes visually uneven exactly where the book names the matrix. | Promoted to `working/10_active/as_figure_production_queue.md` as P0 figure tasks. |
...
| Status | Location | Issue | Why It Matters | Suggested Fix |
|---|---|---|---|---|
| `[ ]` | Main accusation arc: Prologue, Ch1, Ch18, Ch19, Epilogue | The antagonist vocabulary is mostly calibrated. The Prologue names the accused as the asuric pyramid, Ch1 names the two-category split, Ch18 names gaslighting/heroic erasure, Ch19 convicts PIE, and the Epilogue restores Sanskrit's category. | The main polemic now has an accusation-to-verdict structure instead of scattered anger. This should be protected during edits. | Keep this arc intact. Fix local overheat without weakening the frame: category theft, misclassification, codification, heroic erasure, gaslighting, verdict, restoration. |
...
| `[x]` | Ch19 Appendix forward-pointer | Ch19 forwarded to Appendix Part 1 as “institutional cartel” while Appendix Part 1 itself carried unresolved register issues. | The main verdict leans on that appendix as a companion. | Calibrated Appendix Part 1 register and removed the reader-facing cartel framing. |
...
| Status | Location | Issue | Why It Matters | Suggested Fix |
|---|---|---|---|---|
| `[x]` | Epilogue opening | The Epilogue previously opened by pointing from the Prologue to Chapter 19 without naming Ch20's remedy work. | The courtroom loop is correct, but the remedy chapter should not feel skipped in the opening recap. | Added the Ch19 / Ch20 / Epilogue bridge. |
...
| P0 | Endnotes + Appendix Part 1 | Resolve or remove all `[VERIFY]` and `[TBD]` markers that appear in reader-facing material. Audit logged in `working/10_active/as_verification_todo.md` under “Production blocker audit — 2026-05-31.” | Pass 4, Pass 5 | `[x]` |
...
| P1 | Ch12 | Either add a compact *samāsa* treatment or remove “compounds” from the opening promise. Consider trimming §12.9 if vivimorphosis pulls too far into Ch19 territory. | Pass 3, Pass 5 | `[x]` |
...
| P1 | Epilogue opening | Add one bridge that includes Ch20: Ch19 closed prosecution, Ch20 began remedy, Epilogue returns the movement to dharmic ground. | Pass 2, Pass 6 | `[x]` |
