# Epilogue Review - The Atris Find the Sun

The manuscript remains unchanged until a decision is selected here.

## EPI-001 - Reconcile recovery with the residual plates
...
Keep the original wound-line and reconcile it with the residual plates:
...
Keep the first two paragraphs in `The Eclipse Is Over`. Move the full *āryatva* discussion to `The Invitation`, where the mantra and the discipline are explained together.

Retain the original bridge without softening it:

> Recovery is not revenge, because the dharmic account is karmic: action bears consequence, and restoration remains possible when action changes. The proper word is **प्रायश्चित्त (*prāyaścitta*)**: self-initiated atonement, internal accountability, correction by action.
>
> ***Break the shadow. Dispel the invented ancestor. Find the Sun. Invite the world. Restore the standard.***

Move the three following paragraphs on the pyramid's appropriation of *āryatva* to `The Invitation`. Their force belongs there, immediately before the reader is asked to earn the word.
...
## EPI-003 - Tell the samudra-manthana story before interpreting it
...
Add the missing story details while preserving the original analogy and its strongest image:
...
## EPI-004 - Present Svarbhānu as a structural analogy
...
Revise only the claim that the pyramid repeated the story "exactly." An analogy need not pretend to be a literal replay:
...
## EPI-005 - Slow the Mohinī interpretation
...
Frame this as the book's interpretation, then retain the sentences that give the passage its force:
...
## EPI-006 - Turn the nectar into a reproducible research program
...
Keep the original passage through:

> That is the nectar.

Keep the original research invitation as prose. It already states the method with urgency and ends with two strong lines:

> The rest of the ocean stands there, indexed, waiting. PIE no longer gets default parenthood: every starred form becomes a question placed before Sanskrit, not a verdict over Sanskrit.

After that prose, add a short repeatable protocol only if the author wants the epilogue to function as a research brief:
...
Do not replace the invitation with the numbered list; the list should make the invitation usable after the original prose has made it desirable.
...
### Comments
Include the checklist as a to-do


## EPI-007 - Merge the chronology refusal into the architectural contest
...
Keep the separate heading and the original opening and closing hammers:
...
## EPI-008 - Make the Atri invitation gentler and more precise
...
Keep the original account of who bears the first responsibility, but replace the direct designation of the reader as an Atri with the gentler formulation already established in the plan:
