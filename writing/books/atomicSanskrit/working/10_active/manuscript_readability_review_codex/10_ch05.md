# Chapter 5 Review - Siddha and Kārya

The manuscript remains unchanged until a decision is selected here.

## C05-001 - Explain grammar before judging the English term

**Source:** [Chapter 5 opening](/Users/paragtope/projects/writing/books/atomicSanskrit/as_1_05_siddha.md:21)  
**Action:** REPLACE  
**Status:** APPLIED — 2026-07-23  
**Applied:** [as_1_05_siddha.md](/Users/paragtope/projects/writing/books/atomicSanskrit/as_1_05_siddha.md:21) — 2026-07-23  

### Original

> To explain that self-conception, the English word *grammar* is wholly inadequate because it actively imports a script-facing bias. Since *grammar* comes through Latin *grammatica* from Greek *grammatikē technē* (the art of letters), its very root is the Greek *gramma*—a letter or written mark. Consequently, because English inherited that history, the word now also bears the heavy weight of a policing apparatus built specifically to enforce proper usage.
>
> Given this history, calling Pāṇini a "grammarian" is deeply misleading. In the framework of Sanskrit and saṃskṛti, the idea of an authority policing speech is fundamentally alien. Pāṇini was never an enforcer of arbitrary standards, nor was he a technician arranging written glyphs. He was a **वैयाकरणः (*vaiyākaraṇaḥ*)**. The **वैयाकरणाः (*vaiyākaraṇāḥ*)** lineage he belonged to did not invent rules to govern behavior; they decoded the existing architecture of sound so the practitioner could preserve precision.

### Proposed

The English word *grammar* comes through Latin from a Greek expression meaning the art of letters. That history gives the word a script-facing bias, and modern English also uses *grammar* for rules that police accepted usage. Neither sense fully describes Sanskrit **व्याकरणम् (*vyākaraṇam*)**.

The **वैयाकरणाः (*vaiyākaraṇāḥ*)** analyzed, decoded, explained, tested, and taught an existing architecture. Pāṇini belonged to that lineage as a **वैयाकरणः (*vaiyākaraṇaḥ*)**, not as an authority who invented standards for speakers or arranged written glyphs. Calling him a “grammarian” can therefore mislead an English reader, while calling him a codifier reverses his role entirely.

### Decision

- [ ] ACCEPT PROPOSED
- [ ] KEEP ORIGINAL
- [x] USE AUTHOR REVISION
- [ ] REVISE AGAIN
- [ ] DEFER

### Author Revision

```text

To explain that self-conception, the English word *grammar* is wholly inadequate. *Grammar* comes through Latin from a Greek expression meaning the art of letters. That history gives the word a script-facing bias, and modern English also uses *grammar* for rules that police accepted usage. Neither sense fully describes Sanskrit **व्याकरणम् (*vyākaraṇam*)**.

The **वैयाकरणाः (*vaiyākaraṇāḥ*)** analyzed, decoded, explained, tested, and taught an existing architecture. Pāṇini belonged to that lineage as a **वैयाकरणः (*vaiyākaraṇaḥ*)**, not as an authority who invented standards for speakers or arranged written glyphs. Calling him a “grammarian” can therefore mislead an English reader, while calling him a codifier reverses his role entirely.

```

### Comments


## C05-002 - Keep Śākalya in the body and move the catalogue into the note

**Source:** [§5.1, pre-Pāṇinian lineage](/Users/paragtope/projects/writing/books/atomicSanskrit/as_1_05_siddha.md:29)  
**Action:** REPLACE  
**Status:** APPLIED — 2026-07-23  
**Applied:** [as_1_05_siddha.md](/Users/paragtope/projects/writing/books/atomicSanskrit/as_1_05_siddha.md:29) — 2026-07-23  

### Original

> The *vyākaraṇa* discipline extends across a long analytical lineage, with well-known practitioners before and after the central reference point of Pāṇini's **अष्टाध्यायी (*Aṣṭādhyāyī*)**. Pāṇini himself cited earlier **वैयाकरणाः (*vaiyākaraṇāḥ*)**. **Śākalya** (शाकल्य) had already decomposed the Rigvedic *saṃhitā* into constituent *padas* through the **पदपाठ (*padapāṭha*)**. The analytical architecture was operating long before the *Aṣṭādhyāyī* documented it.[NOTE: shakalya-padapatha] **Āpiśali** (आपिशलि), **Kāśyapa** (काश्यप), **Gārgya** (गार्ग्य), **Gālava** (गालव), **Cākravarmaṇa** (चाक्रवर्मण), **Bhāradvāja** (भारद्वाज), **Saunaga** (सौनाग), **Senaka** (सेनक), and **Sphoṭāyana** (स्फोटायन) are not decorative names. They are the documentary trace of a discipline already operating.[NOTE: panini-cites-pre-paninian-vaiyakaranas]

### Proposed

The *vyākaraṇa* discipline extends across a long analytical lineage on both sides of Pāṇini's **अष्टाध्यायी (*Aṣṭādhyāyī*)**. Pāṇini cites earlier **वैयाकरणाः (*vaiyākaraṇāḥ*)**, and **शाकल्य (*Śākalya*)** provides a concrete example of their work: he had already separated the Rigvedic *saṃhitā* into constituent *padas* through the **पदपाठ (*padapāṭha*)**. The analytical architecture was operating before the *Aṣṭādhyāyī* documented it.[NOTE: shakalya-padapatha][NOTE: panini-cites-pre-paninian-vaiyakaranas]

Move Āpiśali, Kāśyapa, Gārgya, Gālava, Cākravarmaṇa, Bhāradvāja, Saunaga, Senaka, and Sphoṭāyana into the existing endnote, where each name can be connected to the evidence for it.

### Decision

- [ ] ACCEPT PROPOSED
- [ ] KEEP ORIGINAL
- [x] USE AUTHOR REVISION
- [ ] REVISE AGAIN
- [ ] DEFER

### Author Revision

```text
The *vyākaraṇa* discipline extends across a long analytical lineage before and after Pāṇini's **अष्टाध्यायी (*Aṣṭādhyāyī*)**. Pāṇini cites earlier **वैयाकरणाः (*vaiyākaraṇāḥ*)**, and **शाकल्य (*Śākalya*)** provides a concrete example of their work: he had already separated the Rigvedic *saṃhitā* into constituent *padas* through the **पदपाठ (*padapāṭha*)**. The analytical architecture was operating before the *Aṣṭādhyāyī* documented it.[NOTE: shakalya-padapatha][NOTE: panini-cites-pre-paninian-vaiyakaranas]

Move Āpiśali, Kāśyapa, Gārgya, Gālava, Cākravarmaṇa, Bhāradvāja, Saunaga, Senaka, and Sphoṭāyana into the existing endnote, where each name can be connected to the evidence for it.

```

### Comments
Avoid using "both sides" when using time as a measure.  I prefer sides for space or ideas, not time.

## C05-003 - Explain Patañjali's opening premise in classroom order

**Source:** [§5.2, siddhe śabdārthasambandhe](/Users/paragtope/projects/writing/books/atomicSanskrit/as_1_05_siddha.md:81)  
**Action:** REPLACE  
**Status:** OPEN  
### Original

> Because *Siddhe* is the locative of *siddha* (meaning "in the established", or "where the established holds") and **शब्दार्थसम्बन्ध (*śabdārthasambandha*)** is the bond between word and meaning, this construction formally announces the premise from which the entire commentary proceeds. While the pyramid’s linguistic account treats this bond as mere convention—produced by usage, maintained by a speech community, and arbitrarily altered by time—Patañjali begins from the absolute opposite position: the bond is already established.

### Proposed

**सिद्धे (*siddhe*)** places the discussion inside what is already established. **शब्दार्थसम्बन्ध (*śabdārthasambandha*)** is the bond between word and meaning. Patañjali therefore begins from an existing bond; he does not begin from speakers negotiating one into existence. The pyramid's account begins elsewhere, with a bond produced by usage and altered by later speech communities.

### Decision

- [ ] ACCEPT PROPOSED
- [ ] KEEP ORIGINAL
- [ ] USE AUTHOR REVISION
- [x] REVISE AGAIN
- [ ] DEFER

### Author Revision

```text

```

### Comments
Still a little abstract.  A little more explanation

## C05-004 - Define the two objects without a compressed contrast chain

**Source:** [§5.3 close](/Users/paragtope/projects/writing/books/atomicSanskrit/as_1_05_siddha.md:111)  
**Action:** REPLACE  
**Status:** OPEN  
### Original

> These two models are not simply two theories of the same object; rather, they define fundamentally different objects. While modern historical linguistics studies *kārya* (language as ongoing production), Patañjalian grammar studies *siddha* (language as established structure). Therefore, to apply *kārya* methods to a *siddha* system is to inevitably study the wrong object using the wrong tools.

### Proposed

These premises produce different objects of study. A *kārya* account follows forms as speakers create and change them. A *siddha* account asks how usage remains aligned with an established bond. A method built for the first object will misdescribe the second if it assumes change before examining the architecture.

### Decision

- [ ] ACCEPT PROPOSED
- [ ] KEEP ORIGINAL
- [ ] USE AUTHOR REVISION
- [x] REVISE AGAIN
- [ ] DEFER

### Author Revision

```text

```

### Comments
Again - feels compressed.  Particularly the opening line.

**Note added 2026-08-12 during commit review:** the manuscript at this
location now reads a third way, matching neither Original nor
Proposed above: "Modern linguistics begins with the premise that
languages evolve. It therefore treats the relationship between a word
and its meaning as something that speakers continually alter through
use. In Sanskrit's categories, modern linguistics studies ***kārya***:
language that speakers continually produce and remake." This change
was already on disk, not made in this pass. Status left OPEN since
this item's own decision record was never marked resolved — recommend
the author confirm this text is the intended final version and update
Status/Decision accordingly, or restore one of the tracked options.
