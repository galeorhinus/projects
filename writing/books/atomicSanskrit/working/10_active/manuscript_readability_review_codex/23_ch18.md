...
Keep the original indictment and narrow the two claims that exceed the evidence:

> **One Sanskrit *dhātu*. The Western philological machinery splits the family.** And the splinter is in plain print on the same reference page.

Show the Sanskrit derivation from ⟪दृश्⟫ to पश्यति (*paśyati*) and then show the comparative account, which assigns *paśyati* to PIE **\*spek-** and other forms in the same paradigm to PIE **\*derḱ-**.[NOTE: wiktionary-pasyati-suppletion] Explain *suppletion* in ordinary language, but remove the universal claim that Pāṇinian architecture never uses it anywhere. Close this specific comparison with:

> In this paradigm, *suppletion* names the split created by reconstruction, not a gap in Sanskrit's derivation.

Move *theory* out of the evidentiary table and present it afterward as a proposed semantic connection. Greek *theōros* connects theory with seeing, so comparison with ⟪दृश्⟫ deserves investigation. The semantic fit is striking; the route of transmission still has to be shown. This preserves the punch of the *dragon* and *spy* evidence without asking the weaker case to bear the same weight.
