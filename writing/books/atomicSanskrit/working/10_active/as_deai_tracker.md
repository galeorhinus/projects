...
5. **Strip** approved blocks: `python3 working/tools/deai_strip.py <file>` (or `--check` to count). Then commit the clean version.
...
| A8 | as_3_09_codification_story | App — Codification Story | 530 | 🟢 swept (clean edits) — 3 fixes: reading-ban ("read as evidence"→"taken as"), metadiscourse ("The distinction matters because…" → substance-led), names-verb ("Sanskrit names entropy"→"marks entropy"). Prosecutorial anaphora + closing hammers protected. Left/flagged: "This is fatal…", "The point is not…The point is sharper:", "reads … through categories" |
| A9 | as_3_10_glossary | Glossary | 389 | 🟢 swept (clean edits) — reference/definitional; 4 controlled-vocab fixes: 2 names-verb ("to name the pyramid"/"to name the relation"→"for"), 1 reading-ban ("reading … as common descent"→"treating"), 1 reading-noun ("the 'codification' reading"→"account") |

---

## Framing-Opener Single-Tell Pass (2026-06-24) — book-wide, doubled, awaiting review

A dedicated pass for ONE tell the general de-AI checklist never targeted: the **short abstract framing-announcement opener** (a short, abstract sentence that *announces/labels* a paragraph — "The hard empirical anchor is Mitanni." / "Old Persian supplies the contrast." / "X follows the same pattern." — instead of leading with substance; uniformity is the damage). New de-AI rule recorded in the `feedback-deai-humanize-prose` memory. The temporary sweep brief was deleted after the run. Run via parallel sub-agents in 4 waves; doubled-redline mode, nothing stripped. **128 framing-opener AISWEEP blocks across the book** (122 added this run + 6 from the Ch 19 pilot on 2026-06-05). These sit ALONGSIDE the general-pass blocks — `deai_strip.py` strips both together.

Per-file new blocks (this pass): preface 4, about_series 2, part_01 2, part_04 1 *(overture, acknowledgements, note_on_notes, part_02/03/05/06/07 = 0)*; seekers 4, oppressors 4, botanical 5, strategic 6, fourth_abrahamic 3, siddha 2, apabhramsa 2, dhatuh 1, adivadya 2, superset 3, mapping_mouth 2, building_dhatuh 3; building_kriya 2, building_vakya 8, preservation 2, calibration 2, aural 2, retroflex 5, wrong_question 7, pie_in_sky 5, life_after_pie 1 (+6 pilot = 7); epilogue 2, baking 2, encyclopaedic 5, audiography 4, inventory_atlas 3, language_factory 6, by_the_numbers 6, vedic_carrier 1, codification_story 4, glossary 0, endnotes 9.

**Flagged — framing openers living INSIDE general-pass rewrites** (couldn't be re-doubled without nesting; catchable after the general pass is stripped, then a quick re-scan): oppressors L52, strategic L44 + L71, siddha L57, adivadya L99, superset L10, mapping_mouth L174 + L243, building_dhatuh L205 + L295 + L348. Also same-shape left as enumeration-setups (protected, author's call): life_after_pie L47 + L98.

_Last updated: 2026-06-05 (Preface→Ch 10 general de-AI swept — doubled redlines, awaiting review; 95 AISWEEP blocks across 16 files. Preface done as author template; Chs 0–10 + front-matter via parallel sub-agents. Review in editor or via git diff, then `python3 working/tools/deai_strip.py <file>` to strip approved blocks.) + 2026-06-24 framing-opener single-tell pass book-wide (see section above)._
