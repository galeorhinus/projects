# Lost and Found — Chapter 9 regional-features material removed 2026-06-08

**Created:** 2026-08-22 (retroactively; the cut was not recorded at the time)
**Source commit:** `1a7cb0f5` — "Ch 8 & Ch 9: figure-caption sync + scare-quote
convention + minor polish", 2026-06-08. 123 insertions, **181 deletions**.
**Recovered from:** `git show 1a7cb0f5 -- '*as_1_09*'`

---

## Why this ledger exists

CLAUDE.md's standing rule is **"No silent cuts — Lost and Found first"**: material
is logged before removal, with its source location and disposition. This removal
was not logged. It was made under a commit message describing "minor polish",
while deleting 181 lines including the whole regional-features table (TABLE 9.1,
added 2026-05-20 by "Ch09: Tier 3 addback").

The cut is the direct cause of the largest single jump in orphaned endnotes:
**2 → 15 on 2026-06-08**. Four endnotes cite claims that lived only in this table
and have had no anchor since:

| Endnote | Claim it supported |
|---|---|
| `pahari-tonal-features` | Pahari tonal features (Himalayan frontier row) |
| `punjabi-tonal-development` | Punjabi three-way lexical tone (Indo-Gangetic row) |
| `bengali-va-ba-merger` | Bengali व/ब labial merger (Eastern subcontinent row) |
| `south-indian-mahaprana-loan-only` | *Mahāprāṇa* "not native (Sanskrit loans only)" (Southern row) |

Nothing was lost permanently — git holds it, and it is reproduced in full below —
but it was unrecoverable *by the ledger*, which is what the rule protects against.

## Disposition — needs an author decision

The current Chapter 9 reads well without the table, and a survey table of this kind
may belong in **Appendix Part 4** (the inventory atlas) rather than the body. Three
options:

1. **Restore to Appendix Part 4.** The four endnotes redeploy there and the regional
   sweep is preserved as evidence. Note App 4 already carries coverage surveys, so
   this would need integrating rather than appending.
2. **Retire the four endnotes**, each marked parked with a pointer to this ledger.
3. **Restore selectively** — the tonal rows (Pahari, Punjabi) make a point Chapter 9
   does not otherwise make, since tone is a feature Sanskrit's grid does not encode.

## Note for the Tamil retroflex question

The Southern row records **"Tamil ற alveolar trill"**. That is the same **ற்** raised
on 2026-08-21 when checking Tamil's liquids against the *varṇamālā* — Tamil
distinguishes **ர்** (tap) from **ற்** (trill) where Sanskrit has the single **र**
cell. The book therefore *had* this observation in the manuscript and lost it. It is
relevant to Chapter 8's superset argument and to the still-open question of **ழ்**,
the retroflex central approximant, which has no Sanskrit equivalent at all.

---

## Recovered material (verbatim, as removed)
