...
**Brahui** ([scatter_brahui.svg](build/scatter_brahui.svg)) — spoken by the Brahui people of Balochistan (Pakistan); a geographically isolated speech community far from the southern-subcontinent languages it is grouped with by the machinery. Heavy Persian / Balochi / Arabic contact has reshaped the inventory: uvular /q/, uvular fricatives /x, ɣ/, pharyngeal /ħ/, the glottal stop /ʔ/, and labio-dental /f/. None of these appear in Gondi, Kui, Kuvi, Kolami, Kolami, or Kurukh. The chart's inventory shape resembles Farsi more than the central-forest-belt languages — a polemic point in itself about what the pyramid's family-tree classification holds together. Cells: 28. Columns lit: 1, 2, 4, 5, 6, 7, 8, 9, 10, 11, 12.

### 7.7 Extended southern set, Toda, Malto, and the Burushaski isolate

Five more languages of the same pyramid-classified cluster as §7.6, plus the famous "isolate" of the northern Pakistan mountains that the machinery has been unable to fit into any family tree after a century of attempts.
...
The mahāprāṇa-strip sensitivity check sharpens the interpretation. Removing Sanskrit's two aspirated stop rows raises the Jaccard score for central and southern base inventories — Korku 0.47 → 0.65, Gondi / Kurukh 0.49 → 0.67, Tamil 0.31 → 0.41 — while lowering the heavily Sanskritic-absorbed inventories: Santali 0.78 → 0.50, Telugu / Kannada 0.76 → 0.50, Malayalam 0.74 → 0.49. The aspirated layer behaves like an engineering layer above a more widely shared base. Full results are in [`../../working/40_reference/research/inventory_atlas_mahaprana_strip_results.md`](../../working/40_reference/research/inventory_atlas_mahaprana_strip_results.md).

What this makes visible: **the machinery classifies Sanskrit and Tamil/Telugu/Kannada/Malayalam in different families** (Indo-European vs Dravidian), yet the inventory data shows them MORE similar to Sanskrit than the pyramid's actual Indo-European languages (English, French, etc.) are. The supposed family boundary the philological machinery draws between Sanskrit and the southern subcontinental languages is not corroborated by the consonant inventory evidence.

The polemic move is not "the inventory rankings prove the family-tree is wrong" — single-dimensional inventory similarity is not a full evolutionary argument. The polemic move is that **the pyramid's family classifications make NO contact with this evidence**. The boundary is purely a discipline-internal classification gesture that the inventory comparisons neither corroborate nor refute. In particular: **whatever the philological reconstruction project found between Sanskrit and English to establish them as same-family** does NOT register in the consonant-inventory dimension, where they are among the most-different pairs in the atlas.

### 10.5 Caveats and what the metric does NOT capture

- **Lexical evidence is not addressed.** The pyramid's case for Indo-European is built on regular sound correspondences in inherited vocabulary, NOT on inventory shape. The metric here is one dimension of evidence; it doesn't refute the pyramid's lexical case.
...
- **Sanskrit vs PIE reconstructions** (1862, 1897, 1927, 1973, modern) — would show the pyramid's reconstructed inventory getting LESS similar to Sanskrit over time, even though the reconstruction is supposed to be Sanskrit's ancestor
- **Sanskrit vs Pashto / Old Persian / Avestan / Tocharian** — would test whether the Indo-Iranian closeness the pyramid posits is visible at the inventory level
