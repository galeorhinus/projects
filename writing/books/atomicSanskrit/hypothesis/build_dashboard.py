...
<!-- crossorigin="use-credentials" is load-bearing, not boilerplate. Chrome
     fetches a web app manifest ANONYMOUSLY by default -- no cookies, even
     for a same-origin URL (the request carries Sec-Fetch-Mode: cors and no
     Cookie header; confirmed in Caddy's access log). This path sits inside
     the /as/private/* oauth2-proxy gate, so a credential-less fetch gets a
     302 to the Google login page instead of the JSON. Chrome then has no
     manifest at all: no display:standalone, so it offers "Add shortcut"
     rather than "Install app", and no manifest icons, so the launcher falls
     back to something other than our mark. This attribute makes Chrome send
     the session cookie. The book's own manifest needs no such attribute
     because /as/manifest.json sits OUTSIDE the gated paths -- which is
     exactly why the book installed correctly while this page did not. -->
<link rel="manifest" href="/as/private/dashboard/manifest.json" crossorigin="use-credentials">
...
// A reader's reply goes straight from their browser to Hypothesis, so
// nothing on this side knows it happened. The page they are looking at
// was generated by the last scheduled build, and the next one is up to
// fifteen minutes away -- so the optimistic append disappears on reload
// and a genuinely-saved reply looks lost. (The owner's composer has no
// such gap: it posts through dashboard_api.py, which re-pulls and
// rebuilds immediately afterwards.)
//
// Letting readers trigger that same server-side rebuild would be the
// obvious fix and is the wrong one -- it re-pulls every group and
// rebuilds every page, so handing that trigger to every reader invites
// a thundering herd for something only their own view needs.
//
// Instead the reply is remembered locally and merged back in on load,
...
function rememberPendingReply(parentId, reply) {
  const list = loadPending();
  list.push({ id: reply.id, parentId, user: reply.user, created: reply.created, text: reply.text });
  savePending(list);
}
...
function mergePendingReplies() {
  ...
  // Anything the build already picked up is no longer pending. Also
  // expire after a day, so a reply that somehow never lands cannot
  // haunt the page forever.
  const known = new Set(DATA.map(d => d.id));
  const cutoff = Date.now() - 24 * 60 * 60 * 1000;
  const keep = pending.filter(p =>
    !known.has(p.id) && new Date(p.created).getTime() > cutoff);
  if (keep.length !== pending.length) savePending(keep);
  for (const p of keep) {
    const parent = DATA.find(d => d.id === p.parentId);
    if (!parent) continue;
    if (!parent.thread) parent.thread = [];
    if (parent.thread.some(m => m.id === p.id)) continue;
    parent.thread.push({ id: p.id, user: p.user, created: p.created, text: p.text });
  }
}
...
async function postReaderReply(id, btnEl) {
  ...
  try {
    ...
    a.thread.push(entry);
    // Survive the reload: this page will not be rebuilt for up to
    // fifteen minutes, and without this the reply vanishes until then.
    rememberPendingReply(a.id, entry);
