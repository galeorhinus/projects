...
static void wifiEventHandler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        uint8_t reason = disc ? (uint8_t)disc->reason : (uint8_t)WIFI_REASON_UNSPECIFIED;
        ...
        // Keep provisioning HTTP server running so the portal can show success

        if (provConfig.onSuccess) {
            provConfig.onSuccess(appState.staIpStr.c_str());
        }

        // Start a timer to automatically shut down the AP
        if (appState.apStarted) {
            ...
            if (err == ESP_OK) {
                ...
                esp_timer_start_once(apShutdownTimer, 300 * 1000 * 1000);
...
// mDNS support removed for this project
...
esp_err_t wifiProvisioningStart(const wifiProvisioningConfig *config)
{
    ...
    char host[32];
    snprintf(host, sizeof(host), "clock75-%02d", mac[5] % 100);
    ...
    // mDNS disabled in this project; keep hostname for logs/UI only.
